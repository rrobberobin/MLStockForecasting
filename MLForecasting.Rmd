...
ggsave(filename=paste0("Descr/DescrFigures/TrainValidationTestQuarterlyPlot_", Sys.Date(), ".png"),
       device="png")
...
bestModels = list(model1, model2, model3)
...
dependentNames = c("returns", "volatility",
...
for(n in 1:length(returnMatrix[Split=="Test"]$fdateYQ)) {
  ...
  ER = predsFactorsMerged[fdateYQ == q & gvkey %in% colnames(invV), c("NNER", "SecondER", "ThirdER", 
  ...
  colnames(weight) = c("NNWeights", "SecondNNWeights", "ThirdNNWeights",
  ...
  print(colSums(optimalPortfolios[,c("NNWeights", "SecondNNWeights", "ThirdNNWeights",
...
optimalPortfolios[, list(sum(NNER * NNWeights), 
                         sum(SecondER * SecondNNWeights),
                         sum(ThirdER * ThirdNNWeights)
                         ), by=fdateYQ]
#optimalPortfolios[, sum(NNER * NNWeights) / sum(volas * NNWeights), by=fdateYQ]
...
#   volas = paste0(n, "Vola")
#   optimalPortfoliosWithRealized[, ":=" (name = sum(excessReturns * NNWeights) / sum(volas * NNWeights)), by=fdateYQ]
# }
...
optimalWeights = function(q){
  ...
  res = c()
  for(n in c("BestModelPred", "SecondModelPred", "ThirdModelPred")){
  ...
  # maxSR.lo.ROI.NN <- optimize.portfolio(R=finalR, portfolio=init.lo.portf, momentargs = momentargs,
  #                                  optimize_method="ROI",
  #                                  maxSR=TRUE, trace=TRUE, search_size = search_size)
  # maxSR.lo.ROI.NN
  #chart.Weights(maxSR.lo.RP.NN)
  expectedSharpe = maxSR.lo.RP.NN$objective_measures$mean / maxSR.lo.RP.NN$objective_measures$StdDev
  print(expectedSharpe)
  
  res = cbind(res, maxSR.lo.RP.NN$weights)
  }
  
  colnames(res) = c("NNWeightsNumerical1", "NNWeightsNumerical2", "NNWeightsNumerical3")
...
optimalPortfoliosWithRealized = merge(optimalPortfoliosWithRealized, optimalPortfoliosFromPortfolioAnalytics, by = c("gvkey", "fdateYQ"), all=T)
...
for(x in targets){
  ...
  newDt = fread(path)[, !c("V1", "Model3", "Model4", "Model5"), with=F]
  
  
  for(n in c("Model1", "Model2", "Model3", "Model4", "Model5", "Ensemble")){
...
# for(n in colnames(dt)){
#    set(dt, i = which(dt[[n]] ==1), j = paste0(n, "Weight"),
#         value = 1/length(gvkey))
# 
#    set(dt, i = which(dt[[n]] ==-1), j = paste0(n, "Weight"),
#         value = -1/length(gvkey))
# }
# 
# 
# dt[Model1==1, ":=" ("Model1Weight" = Model1/length(gvkey)), by=fdateYQ]
# dt[Model1==-1, ":=" ("Model1Weight" = Model1/length(gvkey)), by=fdateYQ]
# #dt[Model1==0, ":=" ("Model1Weight" = 0), by=fdateYQ]
# 
# # dt[Model2==1, ":=" ("Model2Weight" = Model2/length(gvkey)), by=fdateYQ]
# # dt[Model2==-1, ":=" ("Model2Weight" = Model2/length(gvkey)), by=fdateYQ]
# # #dt[Model2==0, ":=" ("Model2Weight" = 0), by=fdateYQ]
# # 
# # dt[Model3==1, ":=" ("Model3Weight" = Model1/length(gvkey)), by=fdateYQ]
# # dt[Model3==-1, ":=" ("Model3Weight" = Model1/length(gvkey)), by=fdateYQ]
# # #dt[Model3==0, ":=" ("Model3Weight" = 0), by=fdateYQ]



# #dt = merge(optimalPortfoliosWithRealized, dt[, c("Model1Weight", "gvkey", "fdateYQ")], by=c("gvkey", "fdateYQ"))
# optimalPortfoliosWithRealized[dt, on = c("gvkey", "fdateYQ"), 
#                               Classification1Weight := i.Model1Weight]

optimalPortfoliosWithRealized = merge(optimalPortfoliosWithRealized, dt, on = c("gvkey", "fdateYQ"))


# optimalPortfoliosWithRealized[dt, on = c("gvkey", "fdateYQ"), 
#                               ":=" (Classification1Weight = i.Model1Weight, 
#                                     Classification2Weight = i.Model2Weight,
#                                     Classification3Weight = i.Model3Weight)]
...
regressionNames = c("NN", "SecondNN", "ThirdNN",
                  "Zero", "Mean", "firmMean", "equal")
...
for(n in allPortfolios){
  ...
  #optimalPortfoliosWithRealized[, paste0(n, "PortfolioReturn") :=  sum(w * monthlyReturns, na.rm = T), by = c("YM")]
  optimalPortfoliosWithRealized[, paste0(n, "PortfolioReturn") :=  sum(get(paste0(n, "Return")), na.rm = T), by = c("YM")]
}


# for(n in allPortfolios){
#   name = paste0(n, "Return")
#   weight = paste0(n, "Weights")
#   optimalPortfoliosWithRealized[, ":=" (name = weight * monthlyReturns)]
# }


# optimalPortfoliosWithRealized[, ":=" (
#   "NNReturn" = NNWeights * monthlyReturns,
#   "NNReturnNumerical" = NNWeightsNumerical * monthlyReturns,
#   "SecondNNReturn" = SecondNNWeights * monthlyReturns,
#   "ThirdNNReturn" = ThirdNNWeights * monthlyReturns,
#   "ClassificationReturn" = Classification1Weight * monthlyReturns,
...
# optimalPortfoliosWithRealized[, ":=" (
#   "NNPortfolioReturn" = sum(NNReturn),
#   "NN2PortfolioReturn" = sum(SecondNNReturn),
#   "NN3PortfolioReturn" = sum(ThirdNNReturn),
#   "NNPortfolioReturnNumerical" = sum(NNReturnNumerical, na.rm = T),
#   "ClassPortfolioReturn" = sum(ClassificationReturn, na.rm = T),
#   "ZeroPortfolioReturn" = sum(ZeroReturn),
#   "MeanPortfolioReturn" = sum(MeanReturn),
#   "FirmMeanPortfolioReturn" = sum(FirmMeanReturn),
#   "EWPortfolioReturn" = sum(EWReturn)
# ), by = c("YM")]

# realizedPortfolios = unique(optimalPortfoliosWithRealized[, c(
#   "YM",
#   "NNPortfolioReturn",
#   "NN2PortfolioReturn",
#   "NN3PortfolioReturn",
#   "NNPortfolioReturnNumerical",
#   "ClassPortfolioReturn",
#   "ZeroPortfolioReturn",
#   "MeanPortfolioReturn",
...
realizedPortfolios = unique(
  optimalPortfoliosWithRealized[, c("YM", paste0(allPortfolios, "PortfolioReturn")), 
                                with=F], by = c("YM"))
...
optimalPortfoliosWithRealized[, list(
  ...
  sum(SecondNNWeights),
  sum(ThirdNNWeights),
  sum(NNWeightsNumerical),
  sum(Classification1Weight),
  sum(MeanWeights),
  sum(firmMeanWeights),
  sum(equalWeights)
), by = c("YM")]
...
# factorsWithReturns[, ":=" (
#   "NNPortfolioER" = NNPortfolioReturn - RF.DM,
#   "NN2PortfolioER" = NN2PortfolioReturn - RF.DM,
#   "NN3PortfolioER" = NN3PortfolioReturn - RF.DM,
#   "NNPortfolioNumericalER" = NNPortfolioReturnNumerical - RF.DM,
#   "ClassPortfolioER" = ClassPortfolioReturn - RF.DM,
...
#   "EWPortfolioER" = EWPortfolioReturn - RF.DM
# ), by = c("YM")]
...
FF6Factors = colnames(monthlyFactors[, !c("V1", "YM"), with =F])
...
allPortfolios = gsub("\\['", "_", allPortfolios)
allPortfolios = gsub("\\']", "", allPortfolios)

formulas = list(); models = list()
for(n in allPortfolios){
  
  f = paste(paste0(n, "ER"), "~", paste(FF6Factors, collapse=" + ")) 
    # `Mkt-RF.DM` + `SMB.DM` + `HML.DM` + `RMW.DM` + `CMA.DM` + `WML.DM` + 
    # `Mkt-RF.EM` + `SMB.EM` + `HML.EM` + `RMW.EM` + `CMA.EM` + `WML.EM`
  
  formulas = c(formulas, list(f))
  models = c(models, list(lm(f, data=factorsWithReturns)))
}
names(formulas) = paste0(allPortfolios, "F")
names(models) = allPortfolios
...
# 
# NN2F = `NN2PortfolioER` ~ `Mkt-RF.DM` + `SMB.DM` + `HML.DM` + `RMW.DM` + `CMA.DM` + `WML.DM` + `Mkt-RF.EM` + `SMB.EM` + `HML.EM` + `RMW.EM` + `CMA.EM` + `WML.EM`
# 
# NN3F = `NN3PortfolioER` ~ `Mkt-RF.DM` + `SMB.DM` + `HML.DM` + `RMW.DM` + `CMA.DM` + `WML.DM` + `Mkt-RF.EM` + `SMB.EM` + `HML.EM` + `RMW.EM` + `CMA.EM` + `WML.EM`
# 
# NNNumericalF = `NNPortfolioNumericalER` ~ `Mkt-RF.DM` + `SMB.DM` + `HML.DM` + `RMW.DM` + `CMA.DM` + `WML.DM` + `Mkt-RF.EM` + `SMB.EM` + `HML.EM` + `RMW.EM` + `CMA.EM` + `WML.EM`
# 
# ClassF = `ClassPortfolioER` ~ `Mkt-RF.DM` + `SMB.DM` + `HML.DM` + `RMW.DM` + `CMA.DM` + `WML.DM` + `Mkt-RF.EM` + `SMB.EM` + `HML.EM` + `RMW.EM` + `CMA.EM` + `WML.EM`
# 
...
# NN2FamaFrench = lm(NN2F, data=factorsWithReturns)
# NN3FamaFrench = lm(NN3F, data=factorsWithReturns)
# NNNumericalFamaFrench = lm(NNNumericalF, data=factorsWithReturns)
# ClassFamaFrench = lm(ClassF, data=factorsWithReturns)
...
# models = list(NNFamaFrench, NN2FamaFrench, NN3FamaFrench, NNNumericalFamaFrench, ClassFamaFrench, MeanFamaFrench, FirmMeanFamaFrench, EWFamaFrench)


#Look at the portfolio returns, Are there huge outliers?
#No, the issue was in the tangency weights
...
VIF1 = VIF[1]
colnames(VIF1) = "VIF"
write.csv(VIF1, "Results/VIF4.csv")
...
diagnostics = list(c("test" = "Jarque-Bera", unlist(lapply(JB, function(x) round(x$p.value, 3)))),
                   c("test" = "Breusch-Pagan", unlist(lapply(BP, function(x) round(x$p.value, 3)))),
                   c("test" = "Breusch-Godfrey", unlist(lapply(BG, function(x) round(x$p.value, 3)))))
...
stargazer::stargazer(models,
                     ...
                     out = "Results/FamaFrenchSixFactorsWithNaiveModels3.html"
                     )
...
stargazer::stargazer(regModels,
                     type="text",
                     report=("vc*p"),
                     add.lines = regDiagnostics,
                     out = "Results/RegressionModels.html"
                     )

stargazer::stargazer(classModels,
                     type="text",
                     report=("vc*p"),
                     add.lines = classDiagnostics,
                     out = "Results/ClassificationModels.html"
                     )
...
stargazer::stargazer(models,
                     ...
                     report=("vc*p"),
                     se  = robustErrors,
                     out = "Results/FamaFrenchSixFactorsWithRobustErrors2.html"
                     )
