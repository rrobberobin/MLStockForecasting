---
title: "Master's Thesis"
author: "Robin Perala"
date: "2022-09-10"
output: html_document
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## R Markdown


```{r libs}


#Check files: RnD, Lab8-Garch, OptimalHedgeRatio, Ex4Part2, nnboston, HW7, Gridsearch, LSTM



# library(torch)
# library(luz) # high-level interface for torch
# library(torchvision) # for datasets and image transformation
# library(torchdatasets) # for datasets we are going to use
# library(zeallot)


# library(dplyr) #Contains "mutate_if" function
# library(magrittr) #Contains pipes, e.g. %<>%
# # library(neuralnet) #For fitting neural networks
# # library(keras) #Contains "keras_model_sequential" function
# library(glmnet) #For regularized linear models (e.g. lasso and ridge)



```


```{ WRDS SSH}

wrds-cloud.wharton.upenn.edu
username = ""
password = ""

qrsh
R --no-save --no-restore

res <- dbSendQuery(wrds, "select * 
                   from comp.g_secd
                   where gvkey = '005302'")
data <- dbFetch(res, n=10)
dbClearResult(res)
data

quit()
logout


```




```{r}

require(RPostgres)
wrds <- dbConnect(Postgres(),
                  host='wrds-pgdata.wharton.upenn.edu',
                  port=9737,
                  dbname='wrds',
                  sslmode='require',
                  user=username)


res <- dbSendQuery(wrds, "SELECT * FROM dataset")
#data <- dbFetch(res, n=-1)
data <- dbFetch(res, n=10)
dbClearResult(res)
data


res <- dbSendQuery(wrds, "select distinct table_schema
                   from information_schema.tables
                   where table_type ='VIEW'
                   or table_type ='FOREIGN TABLE'
                   order by table_schema")
data <- dbFetch(res, n=-1)
dbClearResult(res)
data


res <- dbSendQuery(wrds, "select distinct table_name
                   from information_schema.columns
                   where table_schema='comp'
                   order by table_name")
data <- dbFetch(res, n=-1)
dbClearResult(res)
data


res <- dbSendQuery(wrds, "select column_name
                   from information_schema.columns
                   where table_schema='comp'
                   and table_name='g_fundq'
                   order by column_name")
data <- dbFetch(res, n=-1)
dbClearResult(res)
data


res <- dbSendQuery(wrds, "select * 
                   from comp.g_fundq
                   where gvkey = '005302'")
data <- dbFetch(res, n=10)
dbClearResult(res)
data


res <- dbSendQuery(wrds, "select * 
                   from comp.g_secd
                   where gvkey = '005302'")
data <- dbFetch(res, n=10)
dbClearResult(res)
data





res <- dbSendQuery(wrds, "select * 
                   from comp.g_fundq")
dataCols <- dbFetch(res, n=10)
dbClearResult(res)
dataCols



res <- dbSendQuery(wrds, "select gvkey, datadate, ajexdi 
                   from comp.g_secd")
system.time(
  #data <- dbFetch(res, n=10000000)
  data <- dbFetch(res, n=-1)
  )

dbClearResult(res)
data





require(data.table)
data = as.data.table(data)
View(data[ajexdi<0.0000001, ])

fwrite(data,"Data/OriginalData/CorrectAjexdi.csv", row.names = FALSE)
data2 <- fread("Data/OriginalData/CorrectAjexdi.csv", nThread=4)



```



```{r CompustatFundamentals}

#I should clean data
#Check all variables what they are. Remove unnecessary
#2 414 247 obs. of  408 =>

#Import data. The data includes both industrials and banks
require(data.table); #setDTthreads(0L
data <- fread("Data/OriginalData/FundamentalsQuarterly.csv", nThread=4, select=NULL)

# #orderedData = data[order(names(data))]
# #setcolorder(data, order(names(data)))
# #orderedData = setorderv(data, order(names(data)))[]
# 
# #Look at data
View(data[1:1000,])

#Check if industry codes (GICS) are of type "historical" or "header" (i.e. not historical)
#View(unique(data[1:10000,], by = c("gvkey", "gsubind", "sic", "naics")))
anyDuplicated(
  unique(data[, c("gvkey", "gsubind", "sic", "naics")], by = c("gvkey", "gsubind", "sic", "naics")), 
               by = c("gvkey"))
# names(data)
# 
# #Compare quarterly and YearToDate data. Remove yearly data?
# data[gvkey %in% c(1166, 1491), .(gvkey, pdateq, fdateq, datacqtr, cogsq, cogsy, revtq, revty)]
# View(data[gvkey %in% c(1166, 1491), .(gvkey, pdateq, fdateq, datacqtr, cogsq, cogsy, revtq, revty)])

#Which firms do not have quarters?
quarterIndex = !data$datacqtr==""
sum(!quarterIndex)
#View(data[!quarterIndex, ])
relevantData = data[quarterIndex, ]
#data = Filter(!data$datacqtr=="", data)


variableNames = c("gvkey", "fdateq", "datadate", "datacqtr", "accdq", "acoq", "acoxq", "actq", "ancq", "aoq", "apq", "atq", "capsq", "ceqq", "cheq", "cogsq", "cstkq", "dfxaq", "dlcq", "dlttq", "dpq", "eqrtq", "eroq", "gpq", "ibmiiq", "ibq", "iditq", "intanq", "invtq", "ivaoq", "lcoq", "lcoxq", "lctq", "lltq", "loq", "lseq", "ltmibq", "ltq", "nopioq", "nopiq", "oiadpq", "oibdpq", "piq", "ppentq", "reccoq", "rectoq", "rectq", "rectrq", "req", "revtq", "saleq", "sctq", "seqq", "teqq", "txtq", "xintq", "xoproq", "xoprq", "xsgaq", "loc", "city", "naics", "sic", "acctstdq",  "compstq", "curcdq")

relevantData = relevantData[, ..variableNames]

#Just removed: 
# "idbflag", "fyrc", "sic",
# "gsector", "ggroup", "gind", "gsubind",
# "fic",  "exchg",
# "bsprq", "scfq", "staltq"

# #Inspecting data
# View(data[is.na(data$naics),])



#require(data.table)
#data <- fread("Data/CleanDataQ.csv")

# gvkey = factor(na.omit(data$gvkey))
# #Industry
# sic = factor(na.omit(data$sic))
# naics = factor(na.omit(data$naics))
# gsector = factor(na.omit(data$gsector))
# ggroup = factor(na.omit(data$ggroup))
# gind = factor(na.omit(data$gind))
# gsubind = factor(na.omit(data$gsubind))
# 
# #Location
# exchg = factor(na.omit(data$exchg))
# fic = factor(na.omit(data$fic))
# loc = factor(na.omit(data$loc))
# city = factor(na.omit(data$city))
# 
# #Other
# acctstdq = factor(na.omit(data$acctstdq))
# bsprq = factor(na.omit(data$bsprq))
# compstq = factor(na.omit(data$compstq))
# scfq = factor(na.omit(data$scfq))
# srcq = factor(na.omit(data$srcq))
# staltq = factor(na.omit(data$staltq))
# stko = factor(na.omit(data$stko))
# updq = factor(na.omit(data$updq))
# 
# View(data[data$staltq != ""])
# View(data[data$gvkey == 212145])
# 
# table(acctstdq)
# table(bsprq)
# table(compstq)
# table(scfq)
# table(srcq)
# table(staltq)
# table(stko)
# table(updq)
# 
# #Deletion
# dlrsn = factor(na.omit(data$dlrsn))
# dldte = factor(na.omit(data$dldte))
# ipodate = factor(na.omit(data$ipodate))
# 
# table(dlrsn)
# sum(!is.na(dldte))
# sum(!is.na(ipodate))
# 
# View(data[!is.na(data$dlrsn)])
# View(data[data$gvkey == 13470])
# # Do not include dlrsn or dldte as variables. They include look-ahead bias


missing = sum(is.na(data))
notMissing = sum(!is.na(data))

missing / (missing + notMissing)

#Clean data
sumOfNotNA = colSums(!is.na(relevantData))
View(sumOfNotNA)

#First: Remove columns with over half NA
minimumNumberOfRows = nrow(relevantData)*0.5
lessThanHalfNA = relevantData[, sumOfNotNA > minimumNumberOfRows, with=FALSE]
#View(lessThanHalfNA[1:100,])


# sumOfNotNA2 = colSums(!is.na(data))
# View(sumOfNotNA2)
# 
# minimumNumberOfRows = nrow(data)*0.5
# lessThanHalfNA = data[, sumOfNotNA2 > minimumNumberOfRows, with=FALSE]
# View(lessThanHalfNA[1:1000,])
#names(lessThanHalfNA)

#Second: Remove all rows with NA
dataNoNA = na.omit(lessThanHalfNA)
# View(dataNoNA[1:1000,])
# #78 166 x 137
# 
# nrow(data[!staltq==""])
# View(data[!staltq==""])

fwrite(dataNoNA,"Data/CleanDataQ.csv", row.names = FALSE)



```



```{r GGPlot}


startQuarter = min(data[!data$datacqtr == "", datacqtr])
endQuarter = max(data[!data$datacqtr == "", datacqtr])
dataRange = paste(as.character(startQuarter), "-", as.character(endQuarter))
quarters = length(unique(data$datacqtr))
years = quarters/4

#sortedQuarters = sort(unique(YXfactor$datacqtr))


data$datacqtr = zoo::as.yearqtr(data$datacqtr, format = "%YQ%q")


#Plot for document
require(ggplot2)
ggplot(data = data[datacqtr>"2009 Q3",], aes(x = datacqtr)) +
  geom_bar(stat = "count") +
  labs(title = "Compustat Full Data",
       subtitle = dataRange,
       x = "Quarter", y = "Observations")

yearlyFrequencyOriginalData = table(data$datacqtr)
View(yearlyFrequencyOriginalData)






#Same for cleaned data
cleanData = fread("Data/CleanDataQ.csv")

cleanData$datacqtr = zoo::as.yearqtr(cleanData$datacqtr, format = "%YQ%q")
startQuarter = min(cleanData[!cleanData$datacqtr=="", datacqtr])
endQuarter = max(cleanData[!cleanData$datacqtr=="",datacqtr])
dataRange = paste(as.character(startQuarter), "-", as.character(endQuarter))
quarters = length(unique(cleanData$datacqtr))
years = quarters/4

#sortedQuarters = sort(unique(YXfactor$datacqtr))


cleanData$datacqtr = zoo::as.yearqtr(cleanData$datacqtr, format = "%YQ%q")
require(ggplot2)
ggplot(data = cleanData, aes(x = datacqtr)) +
  geom_bar(stat = "count") +
  labs(title = "Compustat Filtered Data",
       subtitle = dataRange,
       x = "Quarter", y = "Observations")

yearlyFrequencyCleanData = table(cleanData$datacqtr)
View(yearlyFrequencyCleanData)


temporalFrequencyTable = merge(yearlyFrequencyOriginalData, yearlyFrequencyCleanData, 
                               by = "Var1", suffixes = c(".Original", ".Filtered"), all = TRUE)

write.csv(temporalFrequencyTable, "Descr/temporalFrequencyTable.csv")


# #Histogram (BeforeMktCap)
# revenueWinsorized = DescTools::Winsorize(cleanData$revtq, probs = c(0.01, 0.99))
# 
# hist(revenueWinsorized, breaks=1000, xlab="Revenue", main = "Histogram of Revenue")
# # hist(log(cleanData$revtq), breaks=50, xlab="Revenue", main = "Histogram of Revenue")

```



```{r priceData}

#Import price data
require(data.table)
prices <- fread("Data/OriginalData/CompustatPrices.csv", nThread=getDTthreads(verbose), select=NULL)
#221 835 959 x 6


cleanData = fread("Data/CleanDataQ.csv")
#data       final    preliminary
#datadate   fdateq   pdateq
# 20100331 20100503 20100503
# 20100630 20100907 20100801
# 20100930 20101105 20101105
# 20101231 20110330 20110306
# 20110630 20110811 20110730
# 20111231 20120331 20120309
#Check update code in fundamentals data

sum(data$updq==3)
sum(data$updq!=3)
View(data[data$updq!=3,][1:100,])
#nrow(cleanData[cleanData$updq==3,])
#almost all data is final (update code 3, instead of 2)
#I will use "fdateq" as a conservative approach

#prirow is the "primary issue tag". Only use primary issue
newPrices = prices[prices$iid == prices$prirow, -c("iid", "prirow")]

#reduce data size
newPrices = newPrices[newPrices$gvkey %in% cleanData$gvkey,]

#Oldest date in cleanData is 2009/11/30
min(cleanData$fdateq)
newPrices = newPrices[newPrices$datadate>=(min(cleanData$fdateq)-300),]


fwrite(newPrices,"Data/CleanPrices.csv", row.names = FALSE)

```

```{r CheckingForLookAheadBias}
# require(data.table)
# cleanData <- fread("Data/CleanDataQ.csv")
# 
# 
# nrow(cleanData[cleanData$datadate>cleanData$pdateq,])
# nrow(cleanData[cleanData$pdateq>cleanData$fdateq,])
# View(cleanData[cleanData$pdateq>cleanData$fdateq,])
# 
# cleanData$datacqtr = zoo::as.yearqtr(cleanData$datacqtr, format = "%YQ%q")
# nrow(cleanData[cleanData$datacqtr>cleanData$fdateq,])
# 
# cleanData$fdateqZoo = as.character(cleanData$fdateq)
# cleanData$fdateqZoo = as.Date(cleanData$fdateqZoo, format = "%Y%m%d")
# #head(cleanData$fdateqZoo - cleanData$datacqtr)
# 
# require(ggplot2)
# ggplot(data = cleanData, aes(x = fdateqZoo)) +
#   geom_bar(stat = "count") +
#   labs(x = "Date", y = "Observations")
# 
# 
# #Check what the fdateq distribution looks like; to see if the dates are clustered
# #Put all years into one calendar year
# cleanData$fdateqMD = as.character(cleanData$fdateq)
# cleanData$fdateqMD = substr(cleanData$fdateqMD, 5, 8)
# cleanData$fdateqMD = as.Date(cleanData$fdateqMD, format = "%m%d")
# require(ggplot2)
# ggplot(data = cleanData, aes(x = fdateqMD)) +
#   geom_bar(stat = "count") +
#   labs(x = "Date", y = "Observations")
# #View(table(cleanData$fdateqMD))
# 
# View(cleanData[1:1000,])


```




```{r combineFundamentalsAndPrices}

require(data.table)
#newPrices <- fread("Data/CleanPrices.csv")
cleanData <- fread("Data/CleanDataQ.csv")
setkey(newPrices, gvkey, datadate)

#View(newPrices[1:1000,])
#View price data in wide format
#widePrices = dcast(newPrices[,], datadate~gvkey + iid, value.var = "prccd")
#widePrices = dcast(newPrices[,], datadate~gvkey, value.var = "prccd")


fundamentalsData=cleanData
View(cleanData[1:1000,])
#https://stackoverflow.com/questions/35046161/how-to-do-a-data-table-rolling-join
# create an interval in the 'compuStat' data.table


#Rebalancing 1.2, 1.5, 1.8, and 1.11
periodization = function(date){
  YYYY = signif(date, 4)
  MMDD = date %% 10000
  
  if(1100<MMDD) {res = YYYY + 201 + 10000}
  else if(800<MMDD) {res = YYYY + 1101}
  else if(500<MMDD) {res = YYYY + 801}
  else if(200<MMDD) {res = YYYY + 501}
  else {res = YYYY + 201}
  "start" = res
}

start = unlist(lapply(fundamentalsData$fdateq, FUN=periodization))
end = start + 2    #56753 with 1    #65325 with 2     #65 331 with 3     #65 333 with 4    #65 334 with 5   #usually I use 7
fundamentalsData[, `:=` (start=start, end=end)]

#fundamentalsData[, `:=` (start = fdateq + 1, end = fdateq + 300)]
#as.POSIXct(zoo::as.yearqtr(strptime(20221212, format = "%Y%m%d")))


# create a second date in the 'prices' data.table
newPrices[, Date2 := datadate]

# set the keys for the two data tables
setkey(fundamentalsData, gvkey, prirow, start, end)
setkey(newPrices, gvkey, iid, datadate, Date2) #iid is "Issue ID"

# create a vector of columnnames which can be removed afterwards
deletecols <- c("Date2", "start", "end", "prirow", "i.datadate")

# perform the overlap join and remove the helper columns
mergedData <- foverlaps(fundamentalsData, newPrices)[, (deletecols) := NULL]



##alternative: non-equi join
# fundamentalsData[, `:=` (start = fdateq - 100, end = fdateq - 1)]
# mergedData2 = newPrices[fundamentalsData, on = .(gvkey, datadate >= start, datadate <= end)]


mergedData = mergedData[order(mergedData$gvkey, -mergedData$datadate, -mergedData$fdateq),]
finalPriceData = mergedData[!duplicated(cbind(mergedData$fdateq,
                                                mergedData$gvkey)),]

finalPriceData = finalPriceData[order(finalPriceData$gvkey, finalPriceData$fdateq),] 

#Remove non-primary IDs
#finalPriceData = finalPriceData[finalPriceData$iid==finalPriceData$prirow,]

#fwrite(finalPriceData,"Data/finalPriceData.csv", row.names = FALSE)


#Write into word doc:
# StartDate >= ReleaseDate(YYYYMMDD)+1
# && 
# StartDate < ReleaseDate(YYYYMMDD)+100
# Minimum Date => First date where this holds

##Should be: datadate == fdateq + 1
# mergedData = merge(cleanData, removeGVKEY, 
#                    by.x = c("gvkey","fdateq"),
#                    by.y = c("gvkey","datadate"))

#https://stackoverflow.com/questions/16095680/merge-dataframes-on-matching-a-b-and-closest-c
# removeGVKEY <- data.table(removeGVKEY, key = c("gvkey","datadate"))
# cleanData <- data.table(cleanData, key = c("gvkey","fdateq"))
# mergedData <- cleanData[removeGVKEY, roll=-1]

```



```{r calculateReturnsAndMktCap}
#require(data.table)
#finalPriceData <- fread("Data/finalPriceData.csv")

#View price data in wide format
#widePrices = dcast(finalPriceData, datadate~gvkey + iid, value.var = "prccd")
#widePrices = dcast(finalPriceData, datadate~gvkey, value.var = "prccd")


ret = function(price, returnFactor, adjustmentFactor) {
  n = length(price)
  numerator = returnFactor[-1]*price[-1]/adjustmentFactor[-1]
  denominator = (returnFactor[-n]*price[-n]/adjustmentFactor[-n])
  
  (numerator/denominator-1) * 100
}

logret = function(price, returnFactor, adjustmentFactor) {
  n = length(price)
  numerator = returnFactor[-1]*price[-1]/adjustmentFactor[-1]
  denominator = (returnFactor[-n]*price[-n]/adjustmentFactor[-n])
  
  exp(log(numerator)-log(denominator))-1
}


# returnData = finalPriceData
# returnData = returnData[, "returns" := c(ret(prccd, trfd, ajexdi), NA), by = gvkey]
# returnData = returnData[, "MktCap" := MktCapFunction(prccd, cshoc)]
# returnData = returnData[!is.na(returnData$returns),]

require(data.table)
# returnData = fread("Data/CleanPrices.csv")
prices <- fread("Data/OriginalData/CompustatPrices.csv", nThread = 4)

# prices <- fread("Data/OriginalData/CompustatPrices.csv", nThread = 4, 
#                 select = c("ajexdi"), colClasses=c("ajexdi"="character"), nrows=270000)[260000:270000]

returnData = prices[prices$iid == prices$prirow, -c("iid", "prirow")]
returnData[, ":=" ("adjustedPrice" = prccd * trfd / ajexdi)]

#View(returnData[is.infinite(adjustedPrice), ])
#View(prices[ajexdi==0, ])
# keysWithInfValues = unique(returnData[is.infinite(adjustedPrice), gvkey])
#keysWithInfValues = unique(prices[ajexdi==0, gvkey])
# View(returnData[gvkey %in% keysWithInfValues, ])

returnData[, "MktCap" := prccd * cshoc]
returnData[, c("cshoc", "prccd", "trfd", "ajexdi"):=NULL]
colnames(returnData)

# returnData[, ":=" ("returns" = c(ret(prccd, trfd, ajexdi), NA), 
#                    "daysByFirm" = length(datadate)), 
#            by = gvkey]
returnData[, ":=" ("returns" = c(adjustedPrice[-1]/adjustedPrice[-length(adjustedPrice)], NA), 
                   "daysByFirm" = length(datadate)), 
           by = gvkey]


returnData[, "year" := floor(datadate/10^4)]
returnData[, "month" := floor((datadate/10^2) %% 10^2)]
returnData[, "quarter" := ceiling((month-1)/3)]
returnData[quarter == 0, ":=" ("quarter" = 4, "year" = year-1)]
#returnData[, ":=" ("YearQuarter" = paste(year, quarter, sep = "Q")), , by=c("year", "quarter")]
returnData[, ":=" ("YearQuarter" = year + (quarter-1)/4)] #This is faster
returnData[, ":=" ("YearMonth" = year + (month-1)/12)]
View(returnData[1:500,])

returnData[, ":=" ("monthlyReturns" = c((prod(returns/100+1, na.rm = T) - 1)*100), 
                   "monthlyVolatility" = c(sd(returns/100, na.rm = T)*sqrt(252)), 
                   "daysInMonth" = length(datadate)), 
           by = list(gvkey, year, month)]

returnData[, ":=" ("monthlyReturns" =c((prod(returns/100+1, na.rm = T) - 1)*100)), 
           by = list(gvkey, year, month)]


# system.time(returnData[1:10000000, c((prod(returns/100+1, na.rm = T) - 1)*100),
#            by = list(gvkey, year, month)])
# system.time(returnData[1:10000000, c((prod(returns/100+1, na.rm = T) - 1)*100),
#            by = list(gvkey, YearMonth)])
# system.time(returnData[1:10000000, c(adjustedPrice[length(adjustedPrice)]/adjustedPrice[1], NA),
#            by = list(gvkey, YearMonth)])
# system.time(returnData[1:10000000, c(exp(adjustedPrice[length(adjustedPrice)] - adjustedPrice[1])-1, NA),
#            by = list(gvkey, YearMonth)])

# system.time(returnData[1:1000000, c(ret(prccd, trfd, ajexdi), NA), 
#            by = list(gvkey, year, month)])
# system.time(returnData[1:1000000, c(logret(prccd, trfd, ajexdi), NA), 
#            by = list(gvkey, year, month)])
# View(returnData[1:500,])


# returnData[, ":=" ("monthlyReturns" = c(ret(prccd, trfd, ajexdi), NA), 
#                    "monthlyVolatility" = c(sd(returns/100, na.rm = T)*sqrt(252)), 
#                    "daysInMonth" = length(datadate)), 
#            by = list(gvkey, year, month)]

# prod(returnData$returns[1:1000]/100+1, na.rm = T) - 1
# exp(log(returnData$prccd[1001]*returnData$trfd[1001]/returnData$ajexdi[1001])
#     - log(returnData$prccd[2]*returnData$trfd[2]/returnData$ajexdi[2])) - 1


returnData[, ":=" ("quarterlyReturns" = c((prod(returns/100+1, na.rm = T) - 1)*100), 
                   "quarterlyVolatility" = c(sd(returns/100, na.rm = T)*sqrt(252)), 
                   "daysInQuarter" = length(datadate)), 
           by = list(gvkey, YearQuarter)]
# returnData[, ":=" ("yearlyReturns" = c((prod(returns/100+1, na.rm = T) - 1)*100), 
#                    "yearlyVolatility" = c(sd(returns/100, na.rm = T)*sqrt(252)), 
#                    "daysInQuarter" = length(datadate)), 
#            by = list(gvkey, year)]

#View(returnData[1:5000,])
#colSums(is.na(returnData))
#View(returnData[!complete.cases(returnData), ])

#monthlyReturnData = returnData[!is.na(returnData$returns),]
monthlyReturnData = unique(returnData, by=c("gvkey", "year", "month"))
monthlyReturnData = na.omit(monthlyReturnData)
monthlyReturnData = monthlyReturnData[, "totalMonthlyObservations" := length(datadate), by = gvkey]
#duplicated(monthlyReturnData, by=c("gvkey", "yearMonth"))
#View(monthlyReturnData[1:5000,])
colSums(is.na(monthlyReturnData))


fwrite(monthlyReturnData,"Data/monthlyReturnData.csv", row.names = FALSE)

require(data.table)
monthlyReturnData = fread("Data/monthlyReturnData.csv")


# quarterlyReturnData = monthlyReturnData[,c("gvkey", "datadate", "year", "quarter", "curcdd", 
#                                            "quarterlyReturns", "quarterlyVolatility", "MktCap", "YearQuarter")]
quarterlyReturnData = unique(monthlyReturnData, by=c("gvkey", "year", "quarter"))


for(lag in 1:4){
  quarterlyReturnData[, paste0("quarterlyReturnsLagged", lag, "Q") := c(rep(NA, min(lag, length(quarterlyReturns))), 
                                                                       head(quarterlyReturns, -lag)), by = gvkey]
  
  quarterlyReturnData[, paste0("quarterlyVolatilityLagged", lag, "Q") := c(rep(NA, min(lag, length(quarterlyVolatility))),
                                                                          head(quarterlyVolatility, -lag)), by = gvkey]
  print(lag)
}


for(lag in 2:5){
  quarterlyReturnData[, paste0("past", lag, "YearReturn")
                      := (Reduce(`*`, shift(quarterlyReturns/100 + 1, 1:(lag*4), type="lag")) - 1)*100, by = gvkey][]
  
  quarterlyReturnData[, paste0("yearlyReturnsLagged", lag-1, "Y") 
                      := (Reduce(`*`, shift(quarterlyReturns/100 + 1, (1 + (lag-1)*4):(lag*4), type="lag")) - 1)*100, by = gvkey][]
  
  quarterlyReturnData[, paste0("past", lag, "YearVolatility") 
                      := c(NA, RcppRoll::roll_sd(quarterlyReturns[-length(quarterlyReturns)]/100, 
                                                 n = lag*4, fill = NA, na.rm = F, align = c("right"))
                           *sqrt(252)), 
                      by = gvkey][]
  print(lag)
}

#View(quarterlyReturnData[1:5000,])

#View(quarterlyReturnData[1:5000,c("quarterlyReturns", "yearlyReturnsLagged2Y", "past2YearVolatility")])

prod(quarterlyReturnData[3:6]$"quarterlyReturns"/100 + 1) -1
sd(quarterlyReturnData[1:4]$"quarterlyReturns"/100)*sqrt(252)

fwrite(quarterlyReturnData,"Data/returnsData.csv", row.names = FALSE)



#monthlyReturnData = monthlyReturnData[,c("gvkey", "datadate", "year", "month", "monthlyReturns", "MktCap")]
#View(monthlyReturnData[1:5000,])

# duplicates = duplicated(cbind(returnData$gvkey, returnData$iid))
# duplicates[1] = TRUE
# returnData = returnData[duplicates,]



```

```{r combineFundamentalsAndPricesNewVersion}

require(data.table)
returnData <- fread("Data/returnsData.csv", nThread=4)
#cleanData <- fread("Data/CleanDataQ.csv", nThread=4)
cleanData <- fread("Data/OriginalData/FundamentalsQuarterly.csv", nThread=4, select=NULL)
#View(returnData[1:100,])

#Remove firms with large fdateq-datadate discrepancy
cleanData$fdateqObject = as.Date(as.character(cleanData$fdateq), format = "%Y%m%d")
cleanData$datadateObject = as.Date(as.character(cleanData$datadate), format = "%Y%m%d")
cleanData[, "TimeDifferenceInDays" := difftime(fdateqObject, datadateObject)]
#View(cleanData[1:1000,])
tail(sort(cleanData$TimeDifferenceInDays), 50)
# 
# #Create fdateq YearQuarter variable
# 
# #fundamentalsData$fdateqYearQuarter = sapply(fundamentalsData$fdateq, FUN=periodization)

cleanData[, "year" := floor(fdateq/10^4)]
cleanData[, "month" := floor((fdateq/10^2) %% 10^2)]
cleanData[, "quarter" := ceiling((month-1)/3)]
cleanData[quarter == 0, ":=" ("quarter" = 4, "year" = year-1)]

setkey(cleanData, gvkey, year, quarter)

#Does this remove the data with the largest date gaps? (Between fdateq and datadate)
fundamentalsData = unique(cleanData, fromLast=T, by = key(cleanData))
mean(as.numeric(cleanData$TimeDifferenceInDays)); mean(as.numeric(fundamentalsData$TimeDifferenceInDays));
mean(as.numeric(cleanData$TimeDifferenceInDays), na.rm = T); mean(as.numeric(fundamentalsData$TimeDifferenceInDays), na.rm = T);

#View(fundamentalsData[1:100,])
#tail(sort(fundamentalsData$TimeDifferenceInDays), 50)
#hist(as.numeric(fundamentalsData$TimeDifferenceInDays), breaks=100)
#hist(as.numeric(fundamentalsData[TimeDifferenceInDays<100,]$TimeDifferenceInDays), breaks=100)
#fundamentalsData = fundamentalsData[TimeDifferenceInDays<250,]
fundamentalsData = fundamentalsData[TimeDifferenceInDays<100,]
#hist(as.numeric(fundamentalsData$TimeDifferenceInDays), breaks=100)
sum(is.na(cleanData$TimeDifferenceInDays)); sum(is.na(fundamentalsData$TimeDifferenceInDays))
#View(fundamentalsData[TimeDifferenceInDays<1, ])
#hist(as.numeric(fundamentalsData[TimeDifferenceInDays<10,]$TimeDifferenceInDays), breaks=30)


#Combine datasets
setkey(fundamentalsData, gvkey, year, quarter)
setkey(returnData, gvkey, year, quarter)

mergedData = merge(returnData, fundamentalsData, 
                   by = c("gvkey", "year", "quarter"),
                   suffixes = c("_price", "_fund"))
#View(mergedData[1:1000,])
#View(mergedData[curcdd!=curcdq, ])

mergedData = mergedData[curcdd==curcdq, ]

fwrite(mergedData, "Data/mergedDataLarge.csv", row.names = FALSE) #, nThread=4

```

```{r Descriptives}

require(data.table)
cleanData <- fread("Data/cleanData.csv") #Should this whole  chunk should be updated to finalData.csv?

colnames(cleanData)[122]
numsData = cleanData[21:105]


#Descriptive statistics
library(moments)
Skewness = c(skewness(numsData, na.rm=T))
Kurtosis = c(kurtosis(numsData, na.rm=T))
kS = round(cbind(Skewness, Kurtosis), 0)
write.csv(kS, "Descr/kurtSkew.csv")



library(stargazer)
stargazer(numsData,
          type = "text",
          out = "Descr/numsDesc.html",
          title = "Descriptive statistics",
          style = "default", #"aer", "qje"
          flip = F,
          digits = 0,
          digits.extra = 0,
          median=T,
          iqr=T#,
          #add.lines = list(Skewness, Kurtosis)
          )

groupData = cleanData[,123:134]

countryIndustry = table(cleanData[,c("loc", "gsector")]) #GICS industry code
allIndustries = apply(countryIndustry, MARGIN=2, sum)
countryIndustryWithIndSum = rbind(countryIndustry, "All Industries"=allIndustries)
allCountries = apply(countryIndustryWithIndSum, MARGIN=1, sum)
countryIndustry = cbind(countryIndustryWithIndSum, "All Countries"=allCountries)

write.csv(countryIndustry, file = "Descr/countryIndustry.csv")



countryIndustry = table(finalData[,c("loc", "gsector")]) #GICS industry code
allIndustries = apply(countryIndustry, MARGIN=2, sum)
countryIndustryWithIndSum = rbind(countryIndustry, "All Industries"=allIndustries)
allCountries = apply(countryIndustryWithIndSum, MARGIN=1, sum)
countryIndustry = cbind(countryIndustryWithIndSum, "All Countries"=allCountries)

write.csv(countryIndustry, file = "Descr/countryIndustry2.csv")


countryIndustry = table(data[,c("loc", "gsector")]) #GICS industry code
allIndustries = apply(countryIndustry, MARGIN=2, sum)
countryIndustryWithIndSum = rbind(countryIndustry, "All Industries"=allIndustries)
allCountries = apply(countryIndustryWithIndSum, MARGIN=1, sum)
countryIndustry = cbind(countryIndustryWithIndSum, "All Countries"=allCountries)

write.csv(countryIndustry, file = "Descr/countryIndustryOrgFile.csv")


industryNames = c(
  "10 Energy",
  "15 Materials",
  "20 Industrials",
  "25 Consumer Discretionary (Consumer Cyclical)",
  "30 Consumer Staples (Consumer Defensive)",
  "35 Health Care",
  "40 Financials",
  "45 Information Technology",
  "50 Communication Services",
  "55 Utilities",
  "60 Real Estate",
  "Country"
)
colnames(countryIndustryOrg) = industryNames
countryIndustryFormatted = knitr::kable(countryIndustryOrg)


#How many companies? 8450
dim(table(cleanData[,c("gvkey")]))


```

```{r DataImputation}

sort(colSums(!is.na(mergedData)))
View(mergedData[1:20,])

table(mergedData$xcomq)

require(mice)
require(missForest)





```



```{r ImportHereAndFactorize}

require(data.table)
YX <- fread("Data/YX.csv")
#YX <- fread("Data/ScaledWinsRatioData.csv")

#factorizing
YXfactor = YX
# factorCols = c("gvkey", "curcdq", "exchg", "fic", "city", "fyrc",
#         "ggroup", "gind", "gsector", 
#         "gsubind", "idbflag", "loc", 
#         "naics", "sic", "fdateq", "datadate")#, "datacqtr")

factorCols = c("gvkey", "fdateYQ", "Split", "naics", "loc", "city",
              "acctstdq", "compstq")#, "bsprq", "scfq", "staltq")

#YXfactor[, c(factorCols) := lapply(.SD, factor), .SDcols = c(factorCols)]


#YXfactor[, c(factorCols) := lapply(.SD, FUN = function(x) caret::dummyVars(formula= " ~ .", data = x, fullRank=FALSE)), .SDcols = c(factorCols)]

# dmy <- caret::dummyVars(" ~ .", data = YX[, c(factorCols)], fullRank=FALSE)
# YXfactor <- data.frame(predict(dmy, newdata = YX[, c(factorCols)]))

#Create date objects
# YXfactor$fdateq = as.Date(YXfactor$fdateq, format = '%Y%m%d')
# YXfactor$datadate = as.Date(YXfactor$datadate, format = '%Y%m%d')
# YXfactor$datacqtr = zoo::as.yearqtr(YXfactor$datacqtr, format = "%YQ%q")


# #Remove non-numerics
# finalNumericData = Filter(is.numeric, YXfactor[, -c("gvkey", "fdateq", "MktCap")])
# #sum(is.na(finalNumericData))
# finalNumericData = na.omit(finalNumericData)


# # library(dplyr) #Contains "mutate_if" function
# # library(magrittr) #Contains pipes, e.g. %<>%
# # X2 = X
# # X2 %<>% mutate_if(is.factor, as.numeric)
# # X3 <- as.matrix(X2)


```

```{r TrainValidationTestSplit}

#Plot the time series
# require(ggplot2)
# ggplot(data = YXfactor, aes(x = fdateq)) +
#       geom_bar(stat = "count", fill = "green")
# 
# ggplot(data = YXfactor, aes(x = datadate)) +
#       geom_bar(stat = "count", fill = "red")


# #I should change these to fdateq quarters
# startQuarter = min(YXfactor$datacqtr)
# endQuarter = max(YXfactor$datacqtr)
# dataRange = paste(as.character(startQuarter), "-", as.character(endQuarter))
# quarters = length(unique(YXfactor$datacqtr))
# years = quarters/4
# 
# sortedQuarters = sort(unique(YXfactor$datacqtr))


require(data.table)
mergedData <- fread("Data/mergedData.csv")
#mergedData <- fread("Data/mergedDataLarge.csv", nThread=4)
#View(mergedData[1:100,])

# factor(mergedData$naics)
# factor(mergedData$sic)
# table(mergedData$naics)
# table(mergedData$sic)
# sum(is.na(mergedData$naics))
# sum(is.na(mergedData$sic))
# colSums(is.na(mergedData))

mergedData$fdateYQ = paste(mergedData$year, mergedData$quarter, sep="Q")
mergedData$fdateYQ = zoo::as.yearqtr(mergedData$fdateYQ, format="%YQ%q")

#mergedData = na.omit(mergedData)

startQuarter = min(mergedData$fdateYQ)
endQuarter = max(mergedData$fdateYQ)
dataRange = paste(as.character(startQuarter), "-", as.character(endQuarter))
quarters = length(unique(mergedData$fdateYQ))
years = quarters/4

sortedQuarters = sort(unique(mergedData$fdateYQ))

trainValidSplitPoint = round(quarters*0.6)
validTestSplitPoint = round(quarters*0.85)

trainQuarters = sortedQuarters[1:trainValidSplitPoint]
validQuarters = sortedQuarters[(trainValidSplitPoint+1):validTestSplitPoint]
testQuarters = sortedQuarters[(validTestSplitPoint+1):quarters]

tTVSplitFunction = function(x){
  if(x %in% trainQuarters) {"Train"}
  else if(x %in% validQuarters) {"Validation"}
  else {"Test"}
}

mergedData[, "Split" := sapply(fdateYQ, tTVSplitFunction)]
#YXfactor$Split = factor(YXfactor$Split)
#View(mergedData[1:1000,])
table(mergedData[, "Split"]); prop.table(table(mergedData[, "Split"]))
# splitFrequencies = rbind(table(mergedData[, "Split"]), 
#                                prop.table(table(mergedData[, "Split"])))
splitFrequencies = rbind(
  table(mergedData$Split), 
  round(prop.table(table(mergedData$Split)), 2)
                         )
splitFrequencies = format(splitFrequencies[,c("Train", "Validation", "Test")], 
                          scientific = FALSE, trim=T, zero.print =F,  drop0trailing=T)
splitFrequencies

#fwrite(mergedData, "Data/mergedDataWithSplitLarge.csv", row.names = FALSE)

```



```{r TrainValidationTestQuarterlyPlot}

#Plot for document
require(ggplot2)
ggplot(data = mergedData, aes(x = fdateYQ)) +
  geom_bar(stat = "count") +
  labs(title = "Final Data",
       subtitle = dataRange,
       x = "Quarter", y = "Observations") +
  zoo::scale_x_yearqtr(format = '%Y', n = years)


ggplot(data = mergedData, aes(x = fdateYQ, fill = Split)) +
  geom_bar(stat = "count") +
  labs(title = "Train-Validation-Test Quarterly Split",
       subtitle = dataRange, 
       x = "Quarter", y = "Observations") + 
  zoo::scale_x_yearqtr(format = '%Y', n = years) +
  scale_fill_discrete(breaks=c('Train', 'Validation', 'Test')) + 
  theme(plot.title = element_text(size=18)) +
  theme(plot.title = element_text(face = "bold"))

# ggplot(data = mergedData, aes(x = datacqtr, fill = Split)) +
#   geom_bar(stat = "count") +
#   labs(title = "Train-Validation-Test Quarterly Split",
#        subtitle = dataRange, 
#        x = "Quarter", y = "Observations") + 
#   zoo::scale_x_yearqtr(format = '%Y', n = years) +
#   scale_fill_discrete(breaks=c('Train', 'Validation', 'Test'))

ggsave(
  filename=paste0("Descr/DescrFigures/TrainValidationTestQuarterlyPlot_", Sys.Date(), ".png"), 
  device="png", 
  scale=0.8,
  dpi=1000, 
  #width = 5, 
  height = 12, units = "cm"
)



# hist(YXfactor$aoq, breaks=200)
# hist(YXfactor$cogsq, breaks=200)
# hist(YXfactor$lctq, breaks=200)
# 
# 
# #Histograms for document
# hist(YXfactor$revtq, breaks=200, xlab="Revenue", main = "Histogram of Revenue (Winsorized and Scaled)")
# hist(log(YXfactor$revtq), breaks=200, xlab="Ln(Revenue)", main = "Histogram of LogRevenue")
# 
# hist(YXfactor$returns, breaks=200, xlab="Returns", main = "Histogram of Returns (Winsorized and Scaled)")
# hist(log(YXfactor$returns/100 + 1), breaks=200, xlab="Ln(Returns)", main = "Histogram of LogReturns")


```

```{r EncodingForClassification}
require(data.table)
#mergedData <- fread("Data/mergedDataWithSplit.csv")
mergedData <- fread("Data/mergedDataWithSplitLarge.csv", nThread=4)


#mergedData[is.infinite(mergedData)]
mergedData[is.infinite(rowSums(mergedData[, laggedVolAndRetNames, with=F])), laggedVolAndRetNames, with=F]

lapply(mergedData[, laggedVolAndRetNames, with=F], function(x) x[is.infinite(x)])

mergedData[, laggedVolAndRetNames, with=F][Reduce(`&`, lapply(mergedData[, laggedVolAndRetNames, with=F], is.infinite)),]

View(mergedData[, c("returns", "prccd", "trfd", "ajexdi", "adjustedPrice", laggedVolAndRetNames), with=F][
                  rowSums(
                    sapply(
                      mergedData[, c("returns", "prccd", "trfd", "ajexdi", "adjustedPrice", laggedVolAndRetNames), with=F], 
                                 is.infinite)) > 0, ])


#EncodingForClassification
require(dplyr)
mergedData[, ":=" ("2Classes" = ntile(quarterlyReturns, 2), 
                  "4Classes" = ntile(quarterlyReturns, 4), 
                  "6Classes" = ntile(quarterlyReturns, 6), 
                  "8Classes" = ntile(quarterlyReturns, 8), 
                  "10Classes" = ntile(quarterlyReturns, 10)), by = c("YearQuarter")]


mergedData[`2Classes` %in% 1, "Simple2Classes" := 0]
mergedData[`2Classes` %in% 2, "Simple2Classes" := 1]

mergedData[`4Classes` %in% 1, "Simple4Classes" := 0]
mergedData[`4Classes` %in% 2:3, "Simple4Classes" := 1]
mergedData[`4Classes` %in% 4, "Simple4Classes" := 2]

mergedData[`6Classes` %in% 1, "Simple6Classes" := 0]
mergedData[`6Classes` %in% 2:5, "Simple6Classes" := 1]
mergedData[`6Classes` %in% 6, "Simple6Classes" := 2]

mergedData[`8Classes` %in% 1, "Simple8Classes" := 0]
mergedData[`8Classes` %in% 2:7, "Simple8Classes" := 1]
mergedData[`8Classes` %in% 8, "Simple8Classes" := 2]

mergedData[`10Classes` %in% 1, "Simple10Classes" := 0]
mergedData[`10Classes` %in% 2:9, "Simple10Classes" := 1]
mergedData[`10Classes` %in% 10, "Simple10Classes" := 2]


#View(mergedData[1:100,c("gvkey", "YearQuarter", "quarterlyReturns", "2Classes", "4Classes", "6Classes", "8Classes", "10Classes")])

# mergedData[, ":=" ("2Classes" = if(median(quarterlyReturns)>=quarterlyReturns) 2 else 1, 
#                   "4Classes" = ntile(quarterlyReturns, 4)), by = c("YearQuarter")]

# View(mergedData[naics/10^4>=1, ])
# View(mergedData[naics/10^4<=1, ])
# mergedData = mergedData[naics/10^4>1 , naics]

# #NAICS Industry split
# mergedData$Sector = floor(mergedData$naics/10^4)
# mergedData$Subsector = floor(mergedData$naics/10^3)
# mergedData$IndustryGroup = floor(mergedData$naics/10^2)
# mergedData$Industry = floor(mergedData$naics/10)
# mergedData$NationalIndustry = mergedData$naics

mergedData$Sector = as.numeric(substr(mergedData$naics, 1, 2))
mergedData$Subsector = as.numeric(substr(mergedData$naics, 1, 3))
mergedData$IndustryGroup = as.numeric(substr(mergedData$naics, 1, 4))
mergedData$Industry = as.numeric(substr(mergedData$naics, 1, 5))
mergedData$NationalIndustry = as.numeric(substr(mergedData$naics, 1, 6))
#View(mergedData[1:100,])
# 
# factor(mergedData$Sector)
# factor(mergedData$Subsector)
# factor(mergedData$IndustryGroup)
# factor(mergedData$Industry)
# factor(mergedData$NationalIndustry)
# 
# table(mergedData$Sector)
# table(mergedData$Subsector)
# table(mergedData$IndustryGroup)
# table(mergedData$Industry)
# table(mergedData$NationalIndustry)


# View(mergedData[sic/10^2>=1, ])
# View(mergedData[sic/10^2<=1, ])
# mergedData = mergedData[sic/10^2>1 , sic]
# View(mergedData[, c("sic")])
# 
# #SIC Industry split
# mergedData$Industry = floor(mergedData$sic/10^2)
# mergedData$Group = floor(mergedData$sic/10)
# mergedData$Division = mergedData$sic

```


```{r removingExtraVariables}

require(data.table)
originalVariableNames <- colnames(fread("Data/OriginalData/FundamentalsQuarterly.csv", nrows=0, drop=c("datadate")))
allVariableNames = colnames(mergedData)
allVariableNames[!allVariableNames  %in% originalVariableNames]
# originalVariableNames[!originalVariableNames  %in% allVariableNames]

#View(mergedData[1:1000,])

# #Removing Unnecessary variables 
# variableNames = c("returns", "MktCap", "gvkey", "fdateq", "datadate", "datacqtr", "accdq", "acoq", "acoxq", "actq", "ancq", "aoq", "apq", "atq", "capsq", "ceqq", "cheq", "cogsq", "cstkq", "dfxaq", "dlcq", "dlttq", "dpq", "eqrtq", "eroq", "gpq", "ibmiiq", "ibq", "iditq", "intanq", "invtq", "ivaoq", "lcoq", "lcoxq", "lctq", "lltq", "loq", "lseq", "ltmibq", "ltq", "nopioq", "nopiq", "oiadpq", "oibdpq", "piq", "ppentq", "reccoq", "rectoq", "rectq", "rectrq", "req", "revtq", "saleq", "sctq", "seqq", "teqq", "txtq", "xintq", "xoproq", "xoprq", "xsgaq", "loc", "city", "naics", "acctstdq", "bsprq", "compstq", "scfq", "staltq")

crucialNames = c("gvkey", "fdateYQ", "quarterlyReturns", "quarterlyVolatility", 
                 "MktCap", "Split", "TimeDifferenceInDays")

dummyNames = c("loc", "city", "acctstdq", "compstq")
industryNames = c("Sector", "Subsector", "IndustryGroup", "Industry", "NationalIndustry")

numericalNames = c("accdq", "acoq", "acoxq", "actq", "ancq", "aoq", "apq", "atq", "capsq", "ceqq", "cheq", "cogsq", "cstkq", "dfxaq", "dlcq", "dlttq", "dpq", "eqrtq", "eroq", "gpq", "ibmiiq", "ibq", "iditq", "intanq", "invtq", "ivaoq", "lcoq", "lcoxq", "lctq", "lltq", "loq", "lseq", "ltmibq", "ltq", "nopioq", "nopiq", "oiadpq", "oibdpq", "piq", "ppentq", "reccoq", "rectoq", "rectq", "rectrq", "req", "revtq", "saleq", "sctq", "seqq", "teqq", "txtq", "xintq", "xoproq", "xoprq", "xsgaq")

numericalNames2 = c("accoq", "adpacq", "amq",
 "aotq", "apoq", "artfsq", "autxrq", "bcefq", "bctq", "bdiq", "capcstq", "capr1q", "capr2q",
 "capr3q", "caq", "cfbdq", "cfereq", "cfoq", "cfpdoq", "chq", "chsq", "cltq", "dfpacq",
 "ditq", "dpactq", "dptbq", "dptcq", "dvpdpq", "dvrreq", "dvtq", "esubq", "fcaq", "feaq",
 "felq", "gdwlamq", "gdwlq", "iatiq", "ibkiq", "iireq", "iitq", "intcq", "iobdq", "ioiq",
 "ioreq", "ipq", "iptiq", "isgtq", "istq", "ivaeqq", "iviq", "ivptq", "ivstq", "ivtfsq",
 "lcabgq", "lcacuq", "lsq", "mibnq", "mibq", "mibtq", "miiq", "mtlq", "nitq", "oproq",
 "pclq", "prcq", "pstkq", "ptranq", "pvoq", "pvtq", "ratiq", "rawmsmq", "reitq", "risq",
 "rltq", "rvlrvq", "rvtiq", "rvutxq", "saaq", "salq", "sbdcq", "scoq", "scq", "spiq",
 "ssnpq", "stkchq", "tdsgq", "tdstq", "transaq", "tstkq", "txdbq", "unnpq", "xagtq", "xbdtq",
 "xcomiq", "xcomq", "xdvreq", "xioq", "xiq", "xiviq", "xivreq", "xobdq", "xoiq", "xoreq",
 "xppq", "xretq", "xsq", "xstoq", "xstq", "xtq")

laggedVolAndRetNames = c("quarterlyReturnsLagged1Q", "quarterlyReturnsLagged2Q", "quarterlyReturnsLagged3Q", "quarterlyReturnsLagged4Q", 
                         #"quarterlyReturns-1", "quarterlyReturns-2", "quarterlyReturns-3", "quarterlyReturns-4", 
                         "quarterlyVolatilityLagged1Q",  "quarterlyVolatilityLagged2Q", "quarterlyVolatilityLagged3Q", "quarterlyVolatilityLagged4Q", 
                         #"quarterlyVolatility-1",  "quarterlyVolatility-2", "quarterlyVolatility-3", "quarterlyVolatility-4", 
                         "yearlyReturnsLagged1Y", "yearlyReturnsLagged2Y", "yearlyReturnsLagged3Y", "yearlyReturnsLagged4Y", 
                          "past2YearReturn", "past3YearReturn", "past4YearReturn", "past5YearReturn", 
                         "past2YearVolatility", "past3YearVolatility", "past4YearVolatility", "past5YearVolatility")

labelNames = c("2Classes", "4Classes", "6Classes", "8Classes", "10Classes", 
               "Simple2Classes", "Simple4Classes", "Simple6Classes", "Simple8Classes", "Simple10Classes")

newVariableNames = c(crucialNames, dummyNames, industryNames, numericalNames, laggedVolAndRetNames, labelNames)

# newVariableNames[!newVariableNames  %in% originalVariableNames]
# originalVariableNames[!originalVariableNames  %in% newVariableNames]

newVariableNames = c(newVariableNames, numericalNames2)
newVariableNames = newVariableNames[!duplicated(newVariableNames)]

# #YX = lapply(YXfactor, function(x){x[, ..variableNames]})
# #YX = lapply(YX, function(x){na.omit(x)})
# YX = finalData[, ..variableNames]
# finalData = mergedData[, c("year", "quarter", "month", "datadate.price", "datadate.fund", 
#                            "fdateq", "datacqtr", "TimeDifferenceInDays", "fdateqObject", "datadateObject"):=NULL]
finalData = mergedData[, newVariableNames, with=F]
colnames(finalData)[colnames(finalData) == 'quarterlyReturns'] <- 'returns'
colnames(finalData)[colnames(finalData) == 'quarterlyVolatility'] <- 'volatility'
# View(finalData[1:100,])


# #removeMissingCalendarData. I could probably manually add these three instead. A simple script for adding
# #which(finalData$datacqtr=="")
# #finalData[c(12721, 68453, 71789),]
# YX=YX[!YX$datacqtr=="",]

# #Trash??
# #a = factor(YX[1]$'2009Q4'$sic)
# YX[,"gvkey"] = apply(YX[,"gvkey"], MARGIN = 2, factor)


fwrite(finalData,"Data/YXLarge.csv", row.names = FALSE)
#fwrite(finalData,"Data/YX.csv", row.names = FALSE)

#Histogram
#revenueWinsorized = DescTools::Winsorize(finalData$revtq/finalData$MktCap, probs = c(0.01, 0.99))

# hist(revenueWinsorized, breaks=1000, xlab="Revenue", main = "Histogram of Revenue")
# hist(log(cleanData$revtq), breaks=50, xlab="Revenue", main = "Histogram of Revenue")

```

```{r PLMSpeedTest}

YX = data.table::fread("Data/YX.csv")
YXfactor = YX[, !c("MktCap", "Split", "staltq", "bsprq", "scfq")]
YXfactor = na.omit(YXfactor)

factorCols = c("gvkey", "loc", "city", "naics",
        "acctstdq", "compstq",
        "fdateYQ")

YXfactor[, c(factorCols) := lapply(.SD, factor), .SDcols = c(factorCols)]
YXPanel = plm::pdata.frame(YXfactor, index = c("gvkey", "fdateYQ"))

n = names(YXfactor)
exclude = c("returns", "gvkey", "fdateYQ")
f <- as.formula(paste("returns ~", paste(n[!n %in% exclude], collapse = " + ")))

plmSpeedTest = plm::plm(f, effect="twoways", model= "within", data = YXPanel)


#table(index(YXfactor), useNA = "ifany")


```


```{r stationarityAndUnitRoot}
#Stationarity test
plm::purtest(finalNumericData)
sum(is.na(finalNumericData))


#Check biggest return
min(finalData$returns, na.rm = T) #-99.27552
max(finalData$returns, na.rm = T) #4561.336

#Returns stationarity (Will not anyway winsorize these). This is wrong as we have panel data
#fUnitRoots::adfTest(finalData$returns, lags = 252, type = c("nc"), title = NULL, description = NULL)
#plm::purtest(finalNumericData, index="gvkey")


```



```{r TrainTestSplitChecking}

testSet = YXfactor[YXfactor$Split=="Test",]
testSet$datacqtr = factor(testSet$datacqtr)
testSetQuarters = split(testSet, testSet$datacqtr)



#Checking if the data periodization makes sense
# for(n in testSet){
#   print(c(min(n$fdateq), max(n$fdateq)))
# }
# 
# for(n in trainValidation){
#   print(c(min(n$fdateq), max(n$fdateq)))
# }
# 
# 
# #Minimum dates look approximately fine. Maximum dates on the other hand...
# which(trainValidation$'2009Q4'$fdateq == max(trainValidation$'2009Q4'$fdateq))
# 
# #Check this for the initial dataset. Is this an error?
# finalData[finalData$gvkey==285237,]

```


```{r CVfunction}




```

```{r removeRedVariables}


```


```{r Pooling}

drop = c("datacqtr", "fdateq")

sparseTestSet = lapply(testSet, function(x) Matrix::sparse.model.matrix(returns ~ ., x[,!..drop]))

sparseTrainValidation = lapply(trainValidation, function(x) Matrix::sparse.model.matrix(returns ~ ., x[, !..drop]))


# sparseYX = Matrix::sparse.model.matrix(returns ~ ., YX)
# Y = YX$returns


YX = sparseTrainValidation
CVsize = round(length(YX)/2):(length(YX)-1)
CVsize = (length(YX)-2):(length(YX)-1)



YX = lapply(trainValidation, function(x) x[, !..drop])

trainXPools = list()
validXPools = list()

trainYPools = list()
validYPools = list()
for (n in CVsize) {
  #Create training set
  set = YX[1:n]
  pooled = do.call(rbind, set)
  trainX = Matrix::sparse.model.matrix(returns ~ ., pooled)
  #trainXRegular = Matrix::model.matrix(returns ~ ., pooled)
  trainY = pooled$returns
  
  #Created validation set
  valid = YX[n + 1][[1]]
  validX = Matrix::sparse.model.matrix(returns ~ ., valid)
  #validXRegular = Matrix::model.matrix(returns ~ ., valid)
  validY = valid$returns
  
  #remove columns that arent in both data sets
  trainCols = trainX@Dimnames[[2]]
  validCols = validX@Dimnames[[2]]
  trainX = trainX[,  trainCols %in% validCols]
  validX = validX[,  validCols %in% trainCols]
  
  trainXPools = append(trainXPools, list(trainX))
  trainYPools = append(trainYPools, list(trainY))
  
  validXPools = append(validXPools, list(validX))
  validYPools = append(validYPools, list(valid$returns))
}




# sparseTrainPools = lapply(trainPools, function(x) {
#   Matrix::sparse.model.matrix(returns ~ ., x)
# })




# for (n in CVsize) {
#   set = YX[1:n]
#   pooled = do.call(rbind, set)
#   
#   #Create training set
#   trainX = Matrix::sparse.model.matrix(returns ~ ., pooled)
#   trainY = pooled$returns
#   
#   #Created validation set
#   valid = YX[n + 1][[1]]
#   validX = sparse.model.matrix(returns ~ ., valid)
#   validY = valid$returns
#   
#   #remove columns that arent in both data sets
#   validCols = validX@Dimnames[[2]]
#   trainCols = trainX@Dimnames[[2]]
#   trainX = trainX[,  trainCols %in% validCols]
#   validX = validX[,  validCols %in% trainCols]
# }




```

## Model horse-race

```{r OLS}

# OLSData = do.call(rbind, trainValidation)[, -c("fdateq")]
# 
# #check duplicates
# unique = c("gvkey", "datacqtr")
# #dups = OLSData[duplicated(OLSData[,..unique])]
# OLSDataUnique = OLSData[!duplicated(OLSData[,..unique])]
# 
# #n = names(OLSPanel)
# n = names(Filter(is.numeric, OLSDataUnique))
# exclude = c("returns", "gvkey", "datacqtr")
# f <- as.formula(paste("returns ~", paste(n[!n %in% exclude], collapse = " + ")))
# 
# OLSPanel = plm::pdata.frame(OLSDataUnique, index = c("gvkey", "datacqtr"))
# OLSmodel2 = plm::plm(f, effect="twoways", model= "within", data=OLSPanel)
# #OLSsummary = summary(OLSmodel2) #computationally singular -> Because of multicollinearity. No issue; just ignore the message
# #OLSsummary$r.squared
# 
# 
# 
# 
# 
# OLSError = c()
# for (n in 1:validationSize) {
#   trainX = trainXPools[[n]]
#   trainY = trainYPools[[n]]
#   validX = validXPools[[n]]
#   validY = validYPools[[n]]
#   
#   OLSmodel3 = glmnet(
#     x = trainX,
#     y = trainY,
#     standardize = TRUE,
#     lambda = 0
#   )
#   
#   OLSpred = predict(OLSmodel3, newx = validX)
#   OLSMSE = mean((OLSpred - validY) ^ 2)
#   OLSMAE =  mean(abs(OLSpred - validY))
#   
#   OLSError = rbind(OLSError, c("OLSMSE" = lassoMSE, "OLSMAE" = lassoMAE))
# }
# meanCV = apply(OLSError, 2, mean)
# meanCV



```


```{r LassoRidge}

# library(glmnet)
#Use function glmnet (from library glmnet) to perform linear regression with Lasso regularization
#alpha=1 is lasso.



validationSize = length(validXPools)
CVfunction = function(lambda, alpha) {
  lassoError = c()
  for (n in 1:validationSize) {
    trainX = trainXPools[[n]]
    trainY = trainYPools[[n]]
    validX = validXPools[[n]]
    validY = validYPools[[n]]
    
    lassoModel = glmnet(
      x = trainX,
      y = trainY,
      alpha = alpha,
      standardize = TRUE,
      lambda = lambda
    )

    lassoPred = predict(lassoModel, newx = validX)
    lassoMSE = mean((lassoPred - validY) ^ 2)
    lassoMAE = mean(abs(lassoPred - validY))
    
    lassoError = rbind(lassoError, c("lassoMSE" = lassoMSE, "lassoMAE" = lassoMAE))
  }
  meanCV = apply(lassoError, 2, mean)
  meanCV
}


grid = c(10^seq(3,-10, length=5), 0)


MSEMatrix = c()
for (alpha in 1:2) {
  for (lambda in grid) {
    lambdaCV = CVfunction(lambda)
    MSEMatrix = rbind(MSEMatrix,
                      c("lambda" = lambda, "alpha" = alpha, lambdaCV))
  }
}
MSEMatrix


```






```{r PCR}

# library(pls)
# #PCR Regression using function pcr (part of pls library)
# PCRfit=pcr(returns~., data=USData, scale=TRUE, subset=train, validation ="CV")
# 
# 
# 
# CVfunction = function(lambda, alpha) {
#   errorMatrix = c()
#   for (n in 1:validationSize) {
#     trainX = trainXPools[[n]]
#     trainY = trainYPools[[n]]
#     validX = validXPools[[n]]
#     validY = validYPools[[n]]
#     
#     train = cbind(trainX, trainY)
#     
#     PCRfit=pcr(returns~., data=train, scale=TRUE)
# 
#     lassoPred = predict(lassoModel, newx = validX)
#     lassoMSE = mean((lassoPred - validY) ^ 2)
#     lassoMAE = mean(abs(lassoPred - validY))
#     
#     lassoError = rbind(lassoError, c("lassoMSE" = lassoMSE, "lassoMAE" = lassoMAE))
#   }
#   meanCV = apply(lassoError, 2, mean)
#   meanCV
# }
# 
# pcrCV = CVfunction(lambda)
# 
# 
# 
# 
# 
# #Validation plot
# validationplot(PCRfit, val.type="MSEP")
# 
# #Scree plotting
# PVE=PCRfit$Xvar/PCRfit$Xtotvar
# plot(PVE, xlab="Principal Component", ylab="Proportion of Variance Explained", 
#      ylim=c(0,1), type="b")
# plot(cumsum(PVE), xlab="Principal Component", ylab="Cumulative Proportion of Variance Explained", 
#      ylim=c(0,1), type="b")


```




```{r Decision Trees}
# library(haven) #For importing Stata data
# library(tree) #For fitting trees
# library(randomForest) #For bagging and randomforests
# library(gbm) #For boosting


# #Fit a tree using function "tree" (in library "tree")
# treeROA = tree(roa_w~. , USData, subset=train)
# summary(treeROA)
# #treeROA
# 
# #Plot
# plot(treeROA); text(treeROA, pretty=0, cex=0.7, srt=25)
# 
# #MSE
# yhat=predict(treeROA, newdata=USData[-train,])
# ROAtest=USData[-train, "roa_w"]
# plot(yhat, ROAtest$roa_w, ylab = "roa_w")
# abline(0,1)
# mean((yhat-ROAtest$roa_w)^2)












#Bagging
# library(randomForest)
CVsize = (length(YX)-2):(length(YX)-1)

treefunction = function(p, nTree, nodes) {
  errorMatrix = c()
  for (n in CVsize) {
    set = YX[1:n]
    pooled = do.call(rbind, set)
    # trainX = stats::model.matrix(returns ~ ., pooled)
    # trainY = pooled$returns
    # 
    # #Created validation set
    valid = YX[n + 1][[1]]
    #validX = stats::model.matrix(returns ~ ., valid)
    validX = valid[,!"returns"]
    validY = valid$returns
    
    
    #remove columns that arent in both data sets
    trainCols = names(pooled)
    validCols = names(valid)
    pooled = pooled[,  (trainCols %in% validCols), with=FALSE]

    validXCols = names(validX)
    validX = validX[,  (validXCols %in% trainCols), with=FALSE]

    #remove columns that arent in both data sets
    # trainCols = trainX@Dimnames[[2]]
    # validCols = validX@Dimnames[[2]]
    # trainX = trainX[,  trainCols %in% validCols]
    # validX = validX[,  validCols %in% trainCols]
    
    # train = cbind(trainX, "returns"=trainY)
    # train = as.data.table(as.matrix(train))
    
    model = randomForest(
      returns ~ .,
      data = pooled,
      mtry = p,
      ntree = nTree,
      maxnodes = nodes,
      importance = TRUE
    )
    
    pred = predict(model, newx = validX) #something is not working here nrows(pred) is the same as nrows(pooled)
    MSE = mean((pred - validY) ^ 2)
    MAE = mean(abs(pred - validY))
    
    errorMatrix = rbind(errorMatrix, c("MSE" = MSE, "MAE" = MAE))
  }
  meanCV = apply(errorMatrix, 2, mean)
  meanCV
}



MSEMatrix = c()
numOfPreds = c(ncol(pooled)/3, ncol(pooled)-1)
nTrees = c(10, 15)
maxNodes = c(2, 3)
for (p in numOfPreds) {
  for (nTree in nTrees) {
    for (nodes in maxNodes) {
      CV = treefunction(p, nTree, nodes)
      MSEMatrix = rbind(MSEMatrix,
                        c(
                          "predictors" = p,
                          "trees" = nTree,
                          "nodes" = nodes,
                          CV
                        ))
    }
  }
}
MSEMatrix





# #Bagging using the function randomForest,
# #inside the randomForest library. When mtry is for all variables, it is bagging. 
# bagROA = randomForest(roa_w~., USData, subset=train, mtry=ncol(USData)-1, importance =TRUE)
# bagROA
# plot(bagROA)
# 
# #MSE
# yhat=predict(bagROA, newdata=USData[-train,])
# ROAtest=USData[-train, "roa_w"]
# plot(yhat, ROAtest$roa_w)
# abline(0,1)
# mean((yhat-ROAtest$roa_w)^2)










# #Random forests
# 
# set.seed(1)
# #Fit a random forest using the function randomForest,
# #inside the randomForest library. When mtry is less than
# #than all variables in the data, it is a random forests model
# forestROA = randomForest(roa_w~., USData, subset=train, importance = TRUE)
# 
# #Should be done on several different mtry
# #sqrt(ncol(USData))
# #ncol(USData)/3
# 
# forestROA
# plot(forestROA)
# 
# importance(forestROA)
# varImpPlot(forestROA)
# 
# #MSE
# yhat=predict(forestROA, newdata=USData[-train,])
# ROAtest=USData[-train, "roa_w"]
# plot(yhat, ROAtest$roa_w)
# abline(0,1)
# mean((yhat-ROAtest$roa_w)^2)







# #Boosting
# 
# set.seed(1)
# #Boosting using the "gbm" function inside the "gbm" library
# boostROA = gbm(roa_w~., USData[train,], distribution="gaussian", n.trees=500, interaction.depth=4, shrinkage = 0.1)
# #all or only training set? cv.folds? check help(gbm)
# #We use cross-validation to select B.
# 
# 
# boostROA
# summary(boostROA)
# plot(boostROA, i="profit_margin_w"); plot(boostROA, i="ebitda_w")
# 
# #MSE
# yhat=predict(boostROA, newdata=USData[-train,])
# ROAtest=USData[-train, "roa_w"]
# plot(yhat, ROAtest$roa_w)
# abline(0,1)
# mean((yhat-ROAtest$roa_w)^2)


```


```{r NeuralNetwork_Torch}

library(torch)
library(luz) #High-level interface for torch
library(magrittr) #Contains pipes, e.g. %<>%

if(cuda_is_available()){
  device = torch_device("cuda")
} else{
  device = torch_device("cpu")
}
  
#torch_manual_seed(13)

NNfunction = function(type, layers, hidUnits, actvFunc, dropout, l1, l2, epo) {
  errorMatrix = c()
  
  if (type == "LSTM" || type == "GRU")
    source("src/LSTM.R")
  else if (type == "Forward")
    source("src/FeedForwardNN.R")
  else
    source("src/Linear.R")
  
  modnn <- modnn %>%
  setup(
    loss = nn_mse_loss(),
    optimizer = optim_rmsprop, #optim_sgd #optim_rmsprop
    metrics = list(luz_metric_mae())
  ) %>%
    set_hparams(
      input_size = 86,
      layers = layers,
      type = type,
      hidUnits = hidUnits,
      dropout=dropout,
      actvFunc=actvFunc
    ) #3130 #inputSize
  
  
  for(n in 1:validationSize) {
    trainX = trainXNN[[n]]
    trainY = trainYNN[[n]]
    validX = validXNN[[n]]
    validY = validYNN[[n]]
    
    
    #Model Fitting
    
    #batch number seems to automatically be 32. 
    #dataloader_options: Options used when creating a dataloader. See torch::dataloader().
    #shuffle=TRUE by default for the training data and batch_size=32 by default.
    #It will error if not NULL and data is already a dataloader.
    fitted <- modnn %>%
      fit(
        data = list(trainX, trainY),
        valid_data = list(validX, validY),
        epochs = epo,
        verbose = TRUE
        #batch_size=100
        #dataloader_options = c(batch_size=100)
      )
    
    #MSE and MAE (The same ones as above)
    pred = predict(fitted, validX)
    MSE = mean((validY - pred) ^ 2)
    MAE = mean(abs(validY - pred))
    
    errorMatrix = rbind(errorMatrix, c("MSE" = MSE$item(), "MAE" = MAE$item()))
  }
  meanCV = apply(errorMatrix, 2, mean)
  meanCV
}


#Hyper-Parameters
modelType = c("Forward", "LSTM", "GRU")
hiddenLayers = c(2, 3)
hiddenUnits = c(2, 3)
activationFunc = c("relu", "sigmoid") #softmax, etc. https://keras.io/api/layers/activations/
dropOut = c(0.5, 0)
#regGrid = c(10^seq(3,-10, length=4), 0)
epochs = 1 #Just choose best model. => Run for 5 epochs, but if 3rd epoch is better, choose it
inputSize = 86 #ncol(trainXPools[[1]])


NNParams = expand.grid(
  stringsAsFactors = F,
  "type" = c("Forward"),
  "layers" = hiddenLayers,
  "hidUnits" = hiddenUnits,
  "actvFunc" = activationFunc,
  "dropout" = dropOut,
  "epo" = epochs
  # "l1" = regGrid,
  # "l2" = regGrid
)

RNNParams = expand.grid(
  stringsAsFactors = F,
  "type" = c("LSTM", "GRU"),
  "layers" = hiddenLayers,
  "hidUnits" = hiddenUnits,
  "actvFunc" = c("NA"),
  "dropout" = dropOut,
  "epo" = epochs
)

LinearParams = list(
  "type" = "Linear",
  "layers" = 0,
  "hidUnits" = 0,
  "actvFunc" = "NA",
  "dropout" = 0,
  "epo" = 1
)
hyperParams = rbind(LinearParams, NNParams, RNNParams)

validationSize = 2 #length(validXPools)
MSEMatrix = c()
HPsize = nrow(hyperParams)

for(n in 1:HPsize) {
  print("\n")
  cat("Model", n, "out of", HPsize)
  print("\n")
  
  HP = as.list(hyperParams[n,])
  CV = do.call(NNfunction, HP)
  MSEMatrix = rbind(MSEMatrix, c(HP, CV))
}
MSEMatrix
finalResults = as.data.frame(MSEMatrix)
finalResults[,-1] = apply(finalResults[,-1], MARGIN=2, FUN=as.numeric)
finalResults = finalResults[order(finalResults[,"MSE"]),]
finalResults <- apply(finalResults, 2, as.character)

write.csv2(finalResults, "Results/results3.csv", row.names = FALSE)



```


```{r Keras}


# library(tensorflow)
# library(keras)

# # #Model Creation
# # library(keras)
# inputSize = ncol(trainXPools[[1]])
# model <- keras_model_sequential()
# model %>%
#   layer_dense(units = hidUnits,
#               activation = 'actvFunc',
#               input_shape = inputSize) %>%
#   layer_activity_regularization(l1 = l1, l2 = l2) %>%
#   layer_dropout(rate = dropout)  %>%
#   layer_dense(units = 1)
#
# #Model Compilation
# model %>% compile(loss = 'mse',
#                   optimizer = 'rmsprop',
#                   metrics = 'mae')
#
# readline()




# #Model Fitting
# mymodel <- model %>%
#   fit(
#     trainX,
#     trainY,
#     epochs = epochs,
#     batch_size = 3, #32 default. Add as an extra hyper-parameter
#     validation_data = list(validX, validY)
#   )


# # #Prediction
# # model %>% evaluate(validX, validY)
# # pred <- model %>% predict(validX)
#
#
# # #Scatter Plot Original vs Predicted
# # plot(testtarget, pred)
# #
# # #Plot model error for each epoch
# # plot(mymodel)


```

## LoadFactors

```{r FamaFrenchData}

library(data.table)
# factorData <- 
#   list.files(path = "FactorData/", pattern = "*.csv") %>% 
#   purrr::map_df(~fread(.))


#Import factors
developedFactors=fread("FactorData/Developed_ex_US_5_Factors.csv", na.strings=c(getOption("datatable.na.strings","NA"), -99.99))
developedMomentum=fread("FactorData/Developed_ex_US_MOM_Factor.csv", na.strings=c(getOption("datatable.na.strings","NA"), -99.99))
developed = dplyr::iNN1ER_join(x = developedFactors, y = developedMomentum, by = "V1")

emergingFactors=fread("FactorData/Emerging_5_Factors.csv", na.strings=c(getOption("datatable.na.strings","NA"), -99.99))
emergingMomentum=fread("FactorData/Emerging_MOM_Factor.csv", na.strings=c(getOption("datatable.na.strings","NA"), -99.99))
emerging = dplyr::iNN1ER_join(x = emergingFactors, y = emergingMomentum, by = "V1")


allFactors = dplyr::iNN1ER_join(x = developed, y = emerging, by = "V1", suffix = c(".DM", ".EM"))

write.csv(allFactors, "FactorData/allMonthlyFactors.csv", row.names = FALSE)


#allFactors[, `Mkt-RF.DM`+`Mkt-RF.EM`]

#allFactors[, (`Mkt-RF.DM`:WML.DM + `Mkt-RF.EM`:WML.EM)/2]


weightedFactors = (allFactors[, `Mkt-RF.DM`:WML.DM] +  allFactors[,`Mkt-RF.EM`:WML.EM])/2
weightedFactors = cbind(allFactors$V1, weightedFactors)
colnames(weightedFactors) = names(developed)


```


```{r QuarterlyFactors}
toQuarters = function(x) {
  q = ceiling((x %% 100 - 1) / 3)
}

toYears = function(x) {
  as.numeric(substr(x, 1, 4))
} 

toYQ = function(x) {
  Q=toQuarters(x)
  Y=signif(x, 4)
  Y+Q
} 

library(data.table)
factorsWithYQ = cbind(weightedFactors, weightedFactors[, .("Y" = toYears(V1),
                                                           "Q" = toQuarters(V1),
                                                           "YQ" = toYQ(V1))])
factorsWithYQ[Q==0, ":="(Q = 4, Y = Y-1)]
factorsWithYQ$YQ = paste(factorsWithYQ$Y, factorsWithYQ$Q, sep="Q")
factorsWithYQ$YQ = zoo::as.yearqtr(factorsWithYQ$YQ, format="%YQ%q")

# geom = function(x) {
#   exp(mean(log(1+x/100))) - 1
# }

factorProd = function(x) {
  decimal = prod((x/100)+1) - 1
  decimal*100
}


# quarterlyFactors = factorsWithYQ[, .(
#   Mkt = factorProd(`Mkt-RF`),
#   SMB = factorProd(SMB),
#   HML = factorProd(HML),
#   RMW = factorProd(RMW),
#   CMA = factorProd(CMA)
# ), by = YQ]
# 
# 
# quarterlyFactors = factorsWithYQ[, .(
#   Mkt = factorProd(`Mkt-RF`),
#   SMB = factorProd(SMB),
#   HML = factorProd(HML),
#   RMW = factorProd(RMW),
#   CMA = factorProd(CMA)
# ), by = .(Y,Q)]


YQ = zoo::as.yearqtr(as.character(weightedFactors$V1), format = "%Y%m")
factorsWithYQ = cbind(weightedFactors, YQ)

quarterlyFactors = factorsWithYQ[, .(
  Mkt = factorProd(`Mkt-RF`),
  SMB = factorProd(SMB),
  HML = factorProd(HML),
  RMW = factorProd(RMW),
  CMA = factorProd(CMA),
  WML = factorProd(WML),
  RF  = factorProd(RF),
  RFpred = ((RF[1]/100+1)^(length(RF))-1)*100
), by = YQ]



write.csv(quarterlyFactors, "FactorData/quarterlyFactors.csv", row.names = FALSE)


```

## Abnormal returns

```{r LoadKerasModelFromPython}

fs::dir_tree("Results/['returns']/Ordinary/SavedKerasModels/BestKerasModel1")

#library(tensorflow)
library(keras)

# target = "['returns']"
# models = c()
# for(n in 1:5){
#   model <- keras::load_model_tf(sprintf("Results/%s/Ordinary/SavedKerasModels/BestKerasModel%s", target, n))
#   models = c(models, model)
# }

model1 <- keras::load_model_tf("Results/['returns']/Ordinary/SavedKerasModels/BestKerasModel1")
model2 <- keras::load_model_tf("Results/['returns']/Ordinary/SavedKerasModels/BestKerasModel2") #load second best model
model3 <- keras::load_model_tf("Results/['returns']/Ordinary/SavedKerasModels/BestKerasModel3") #load third best model
StandardLinearModel <- keras::load_model_tf("Results/['returns']/Ordinary/SavedKerasModels/KerasLinearModel") #load linear model

# Check its architecture
#summary(models[[1]])
summary(model1)
summary(model2)
summary(model3)
summary(StandardLinearModel)

bestModels = list(model1, model2, model3, StandardLinearModel)

# #Model Fitting
# mymodel <- model %>%
#   fit(
#     trainX,
#     trainY,
#     epochs = epochs,
#     batch_size = 32, #32 default. Add as an extra hyper-parameter
#     validation_data = list(validX, validY)
#   )




#   pooled = do.call(rbind, trainXNN)
#   pooled = do.call(rbind, trainYNN)
#   pooled = do.call(rbind, validXNN)
#   pooled = do.call(rbind, validYNN)


#calculate variances
  
  
  


```

```{r ExportModelComparisonTable}

# modelComparisonTable = data.table::fread("Results/ModelComparison.csv")
# 
# 
# write.csv(modelComparisonTable, "Results/ModelComparisonTableInR.csv")


```


```{r Predict}
require(data.table)
#ScaledWinsRatioData <- fread("Data/ScaledWinsRatioData.csv")

YXfactor <- fread("Data/dummiesData.csv")
colSums(YXfactor[,Simple2Classes:Simple10Classes])


library(magrittr)
library(keras) #Contains "keras_model_sequential" function

# YXfactor[acctstdq =="", acctstdq :=NA]
# YXfactor[compstq =="", compstq :=NA]
# YXfactor[city =="", city :=NA]

# YXfactor$acctstdq = factor(YXfactor$acctstdq)
# YXfactor$compstq = factor(YXfactor$compstq)
# YXfactor$city = factor(YXfactor$city)

testSet = YXfactor[YXfactor$Split=="Test",-c("MktCap", "Split")]#, "staltq")]#, "staltq", "bsprq", "scfq")]



#[numCols + dummyNames + ["Split", "fdateYQ"]]
#remove: dependentNames + descrNames + detailedDummyNames
descrNames = c("Split", "MktCap", "compstq")
dependentNames = c("volatility",
                  "2Classes", "4Classes", "6Classes", "8Classes", "10Classes",
                  "Simple2Classes", "Simple4Classes", "Simple6Classes", "Simple8Classes", "Simple10Classes")
detailedDummyNames = c("city", "Subsector", "IndustryGroup", "Industry", "NationalIndustry")

testSet = YXfactor[YXfactor$Split=="Test",-c(descrNames, dependentNames, detailedDummyNames), with=F]


# c(crucialNames, dummyNames, industryNames, numericalNames, laggedVolAndRetNames, labelNames, numericalNames2)

#testSet$fdateYQ = factor(testSet$fdateYQ)
#View(as.data.frame(colnames(YXfactor)))

#View(testSet[acctstdq =="" | compstq =="" | city ==""])
#testSet = testSet[acctstdq !="" & compstq !="" & city !=""]

# View(as.data.frame(levels(testSet$loc)))
# View(as.data.frame(levels(testSet$city)))
# View(as.data.frame(levels(testSet$naics)))
# View(as.data.frame(levels(testSet$acctstdq)))
# View(as.data.frame(levels(testSet$compstq)))

#testSet$datacqtr = factor(testSet$datacqtr)
#testSet$datacqtr = as.numeric(testSet$datacqtr)
#testSetNumericData = Filter(is.numeric, testSet[, -c("gvkey", "fdateq", "MktCap")])
#sum(is.na(finalNumericData))
#testSetNumericData = na.omit(testSetNumericData)

#testSetQuarters = split(testSetNumericData, testSetNumericData$datacqtr)
#testSetQuarters = split(testSet, testSet$datacqtr)


quarters = levels(as.factor(testSet$fdateYQ))
#Prediction
allPreds = c(); m = 1
allPerformances = list(); m = 1
for(q in quarters){
  
  cat("Quarter: ", q)
  test = testSet[testSet$fdateYQ==q, ]
  testX = as.matrix(test[,!c("returns", "fdateYQ", "gvkey")])
  #testX = as.matrix(test[,!c("fdateYQ", "gvkey")])
  testY = test$returns
  
  modelPerformance = as.data.frame(lapply(bestModels, function(x) x %>% evaluate(testX, testY))) #verbose=F
  pred = as.data.table(lapply(bestModels, function(x) x %>% predict(testX))) #verbose=F
  
  colnames(modelPerformance) = c("BestModel", "SecondBestModel", "ThirdBestModel", "LinearModel")
  colnames(pred) = c("BestModelPred", "SecondModelPred", "ThirdModelPred", "LinearModelPred")

  # modelPerformance = model %>% evaluate(testX, testY)
  # pred <- model %>% predict(testX)
  #colnames(pred)[1] = "predictedReturn"
  
  firmPreds = cbind(test[,c("gvkey", "fdateYQ")], pred)
  allPreds = rbind(allPreds, firmPreds)
  
  qName = paste("Quarter:", q)
  allPerformances = c(allPerformances, list(modelPerformance))
  names(allPerformances)[m] = qName
  m = m + 1
}

#View(allPerformances)


allPerformancesAllQ = as.data.frame(
  lapply(bestModels,
         function(x) x %>% evaluate(as.matrix(testSet[,!c("returns", "fdateYQ", "gvkey")]),
                                    testSet$returns))) #verbose=F
colnames(allPerformancesAllQ) = c("BestModel", "SecondBestModel", "ThirdBestModel", "LinearModel")

naiveZeroMSE = sum(testSet$returns^2)/length(testSet$returns)
naiveZeroMAE = sum(abs(testSet$returns))/length(testSet$returns)
NaiveZero = c(NA, naiveZeroMAE, naiveZeroMSE)

allPerformancesAllQWithNaive = cbind(allPerformancesAllQ, NaiveZero)

#testSetQuarters = split(testSet, testSet$fdateYQ)
#testSet = testSetQuarters
# #Prediction
# allPreds = list(); m = 1
# for(n in testSet){
#   
#   #cat("Model number", m)
#   cat("Quarter number", m)
#   #pooled = Filter(function(x) {is.numeric(x)}, n) #|is.factor(x)
#   
#   # pooled = n
#   # testX = pooled[, !c("fdateYQ", "gvkey")]  #c("gvkey", "fdateq", 
#   # testX = stats::model.matrix(returns ~ ., testX)[,-1]
#   testX = as.matrix(n[,!c("returns", "fdateYQ", "gvkey")])
#   #testX = apply(testX, MARGIN = 2, scale)
#   testY = n$returns
#   
#   
#   # testX = n$returns
#   # testY = n$returns
#   model %>% evaluate(testX, testY)
#   pred <- model %>% predict(testX)
#   
#   firmPreds = cbind(n[,c("gvkey", "fdateYQ")], pred)
#   allPreds = c(allPreds, list(firmPreds))
#   m = m + 1
# }



#Scatter Plot Original vs Predicted
#plot(pred, testY, log = "x")

#Plot model error for each epoch
#plot(model)


```

```{r Combine predictions and factors}

quarterlyFactors=fread("FactorData/quarterlyFactors.csv")

#Extract correct risk-free return
#cqtr = max(firmPreds$datacqtr)

# cqtr = lapply(allPreds, function(x) max(x$datacqtr))
# cqtr = lapply(cqtr, function(x) zoo::as.yearqtr(x, format = "%YQ%q"))

# cqtr = lapply(allPreds, function(x) max(x$fdateYQ))
# cqtr = lapply(cqtr, function(x) zoo::as.yearqtr(x, format = "%YQ%q"))

# cqtr = lapply(allPreds, function(x) max(x$fdateYQ))
# cqtr = zoo::as.yearqtr(allPreds, format = "%YQ%q")

# dateTime = as.POSIXct(cqtr, format="%Y%m")
# yearMonth = as.numeric(format(dateTime, format="%Y%m"))
# yearMonth = as.numeric(format(dateTime, format="%Y%m"))


#allPreds$fdateYQ = zoo::as.yearqtr(allPreds$YQ, format = "%Y Q%q")
cqtr = unique(zoo::as.yearqtr(allPreds$fdateYQ))
quarterlyFactors$YQ = zoo::as.yearqtr(quarterlyFactors$YQ, format = "%Y Q%q")
#rf0 = quarterlyFactors[YQ %in% cqtr]$RF
#rf0 = quarterlyFactors[YQ %in% cqtr]$RFpred
rf0 = quarterlyFactors[YQ %in% cqtr]$RFpred

predsFactorsMerged = merge(allPreds, quarterlyFactors, by.x = c("fdateYQ"), by.y = c("YQ"))
predsFactorsMerged$ZeroPred = 0


YX = fread("Data/YX.csv")
YX = YX[, c("gvkey", "fdateYQ", "returns")]
setorder(YX, "fdateYQ", "gvkey")
#predsFactorsMerged[, ":=" ("window" = .GRP), by = c("fdateYQ")]
#predsFactorsMerged[, ":=" ("window" = ), by = c("fdateYQ")]
#YX[, ":=" ("meanPred" = mean(returns, na.rm = T)), by = c("fdateYQ"), roll = -Inf]
#predsFactorsMerged[, ":=" ("meanPred" = frollmean(returns, na.rm = T)), by = c("fdateYQ")]
#predsFactorsMerged[, ":=" ("firmMeanPred" = mean(returns, na.rm = T)), by = c("gvkey", "fdateYQ")]
#YX[, ":=" ("firmMeanPred" = mean(returns, na.rm = T)), by = c("gvkey"), roll = -Inf]

YX[, ":=" ("meanPred" = 0)]
for(q in unique(YX$fdateYQ) ){
  window = YX[YX$fdateYQ<q, c("returns")]
  if(nrow(window) == 0) ans = NA
  else ans = mean(window$returns)
  YX[YX$fdateYQ==q, ":=" ("meanPred" = ans)]
}

YX[, ":=" ("firmMeanPred" = c(NA, cumsum(returns) / seq_along(returns))[-length(returns)]), by = c("gvkey")]
predsFactorsMerged = merge(predsFactorsMerged, YX, by.x = c("fdateYQ", "gvkey"), by.y = c("fdateYQ", "gvkey"))

predsFactorsMerged[, ":=" ("NN1ER"=BestModelPred - RFpred, 
             "NN2ER"=SecondModelPred - RFpred, 
             "NN3ER"=ThirdModelPred - RFpred, 
             "LinearER"=LinearModelPred - RFpred, 
             "ZeroER"=ZeroPred - RFpred, 
             "FirmMeanER"=firmMeanPred - RFpred,
             "MeanER"=meanPred - RFpred)]

# for(n in 1:nrow(YX) ){
#   window = YX[YX$fdateYQ < YX[n,]$fdateYQ & YX$fdateYQ==YX[n,]$gvkey, c("returns")]
#   if(nrow(window) == 0) ans = NA
#   else ans = mean(window$returns)
#   YX[YX$fdateYQ == YX[n,]$fdateYQ & YX$gvkey == YX[n,]$gvkey, ":=" ("firmMeanPred" = ans)]
#   if(n%%1000 == 0) print(n)
# }

```


```{r DieboldMariano}


#MSE-F

#OOS R-squared  = MSE / MSE0 = (compare best model to NaiveZeroPrediction)

#predsFactorsMerged
predNames = c("BestModelPred", "SecondModelPred", "ThirdModelPred", "LinearModelPred", 
              "ZeroPred", "meanPred", "firmMeanPred")
predsForDM = na.omit(predsFactorsMerged[, c(predNames, "returns"), with=F])


predsMSE = apply(predsForDM[, ..predNames], MARGIN=2, FUN= function(x) mean((x-predsForDM$returns)^2))
predsMAE = apply(predsForDM[, ..predNames], MARGIN=2, FUN= function(x) mean(abs(x-predsForDM$returns)))


OOS_R2_Zero = 1 - predsMSE / predsMSE["ZeroPred"]
OOS_R2_Mean = 1 - predsMSE / predsMSE["meanPred"]
OOS_R2_FirmMean = 1 - predsMSE / predsMSE["firmMeanPred"]
OOS_R2_Linear = 1 - predsMSE / predsMSE["LinearModelPred"]
#OOS_R2_Linear = 1 - predsMSE[setdiff(names(predsMSE), "LinearModelPred")] / predsMSE["LinearModelPred"]

OOS_RA_Zero = 1 - predsMAE / predsMAE["ZeroPred"]
OOS_RA_Mean = 1 - predsMAE / predsMAE["meanPred"]
OOS_RA_FirmMean = 1 - predsMAE / predsMAE["firmMeanPred"]
OOS_RA_Linear = 1 - predsMAE / predsMAE["LinearModelPred"]
#OOS_RA_Linear = 1 - predsMAE[setdiff(names(predsMAE), "LinearModelPred")] / predsMAE["LinearModelPred"]

predictionAccuracySummary = rbind(OOS_R2_Zero, OOS_R2_Mean, OOS_R2_FirmMean, OOS_R2_Linear,
                                  OOS_RA_Zero, OOS_RA_Mean, OOS_RA_FirmMean, OOS_RA_Linear)
predictionAccuracySummary[c("OOS_R2_Linear", "OOS_RA_Linear"),"LinearModelPred"] = NA

errorsForDM = predsForDM[, ..predNames] - predsForDM$returns

#Something like this (pseudocode):
#allPerformances["BestModelMSE"] / allPerformances["NaiveZeroMSE"]


namesForDM = c("BestModelPred", "SecondModelPred", "ThirdModelPred", "LinearModelPred")
DMtestZero = apply(
  errorsForDM[, ..namesForDM], MARGIN=2, 
  FUN = forecast::dm.test, 
  errorsForDM$ZeroPred, 
  alternative = c("two.sided"), 
  h = 1,
  power = 2
)
DMtestZeroA = apply(
  errorsForDM[, ..namesForDM], MARGIN=2, 
  FUN = forecast::dm.test, 
  errorsForDM$ZeroPred, 
  alternative = c("two.sided"), 
  h = 1,
  power = 1
)

DMtestMean = apply(
  errorsForDM[, ..namesForDM], MARGIN=2, 
  FUN = forecast::dm.test, 
  errorsForDM$meanPred, 
  alternative = c("two.sided"), 
  h = 1,
  power = 2
)
DMtestMeanA = apply(
  errorsForDM[, ..namesForDM], MARGIN=2, 
  FUN = forecast::dm.test, 
  errorsForDM$meanPred, 
  alternative = c("two.sided"), 
  h = 1,
  power = 1
)

DMtestLinear = apply(
  errorsForDM[, list(BestModelPred, SecondModelPred, ThirdModelPred)], MARGIN=2, 
  FUN = forecast::dm.test, 
  errorsForDM$LinearModelPred, 
  alternative = c("two.sided"), 
  h = 1,
  power = 2
)
DMtestLinearA = apply(
  errorsForDM[, list(BestModelPred, SecondModelPred, ThirdModelPred)], MARGIN=2, 
  FUN = forecast::dm.test, 
  errorsForDM$LinearModelPred, 
  alternative = c("two.sided"), 
  h = 1,
  power = 1
)

DMtests = list(
  "DMZero"=DMtestZero, "DMMean"=DMtestMean, "DMLinear"=DMtestLinear,
  "DMZeroA"=DMtestZeroA, "DMMeanA"=DMtestMeanA, "DMLinearA"=DMtestLinearA
               )

DMPValues = lapply(DMtests, FUN=lapply, function(x) x$p.value[[1]])
DMStatistics = lapply(DMtests, FUN=lapply, function(x) x$statistic[[1]])
#DMPValues = lapply(DMPValues, FUN=unlist)
#a = unlist(DMPValues)
#a = as.data.frame(DMPValues)

DMPValues = as.data.frame(do.call(rbind, DMPValues)); DMPValues[c("DMLinear", "DMLinearA"), "LinearModelPred"] = NA
DMStatistics = as.data.frame(do.call(rbind, DMStatistics)); DMStatistics[c("DMLinear", "DMLinearA"), "LinearModelPred"] = NA

#DMPValues = lapply(DMtests, FUN = function(x) x$p.value)





#install.packages("remotes")
remotes::install_github("grayclhn/oosanalysis-R-library")
dmw_calculation(error.model1^2 - error.model2^2,
                  cbind(error.model1, error.model2, error.model2 * x),
                  R = R, vcv = var)



# clarkwest(null, alt, dataset, R, vcv = var,
#           window = c("rolling", "recursive", "fixed"))
# 
# clarkwest_calculation(target, null.forecast, alt.forecast, vcv)




```



```{r TangencyPortfolio}




#sharpeMaximizingWeights = function(inputPredReturn){#################################################################

#trainValidation = YXfactor[YXfactor$Split!="Test", !c("MktCap")]
pooledForVariance = YXfactor[YXfactor$Split!="Test", c("gvkey", "fdateYQ", "returns", "Split")]

# #For creating a covariance matrix
# pooledForVariance = do.call(rbind, trainValidation)
# pooledForVariance = as.data.table(pooledForVariance[, c("gvkey", "datacqtr", "returns")])
# # pooledForVariance = pooledForVariance[gvkey %in% firmPreds$gvkey,]
# #
# # returnMatrix<-dcast.data.table(data=pooledForVariance, datacqtr~gvkey)
# # dim(returnMatrix[,!"datacqtr"])
# # length(as.matrix(returnMatrix))
# # sum(is.na(returnMatrix))
# #
# # V = cov(returnMatrix[, !"datacqtr"])


# a = dcast.data.table(data=pooledForVariance, datacqtr~gvkey, value.var="returns")
# b = duplicated(pooledForVariance[,-c(returns)])
# d = pooledForVariance[b,]
# sum(duplicated(d[,-c(returns)]))


#For removing the possibility of duplicates
pooledForVariance = pooledForVariance[, "returns" := mean(returns, na.rm = T), by = c("gvkey", "fdateYQ")]
pooledForVariance = unique(pooledForVariance)

allPredsList = split(allPreds, allPreds$fdateYQ)
pooledForVarianceList = lapply(allPredsList, function(x) pooledForVariance[gvkey %in% x$gvkey,])
returnMatrixList = lapply(pooledForVarianceList, function(x) dcast.data.table(data=x, fdateYQ~gvkey, value.var="returns"))
VList = lapply(returnMatrixList, function(x) cov(x[, !"fdateYQ"], use="pairwise.complete.obs"))
# #VList = lapply(returnMatrixList, function(x) cov(x[, !"fdateYQ"], use="complete.obs"))

#returnMatrix = dcast.data.table(data=pooledForVariance, gvkey~fdateYQ+Split, value.var="returns")
pooledForVariance = YXfactor[, c("gvkey", "fdateYQ", "returns", "Split")]
returnMatrix = dcast.data.table(data=pooledForVariance, fdateYQ+Split~gvkey, value.var="returns")




#sharpeMaximizingWeights = function(quarter){#################################################################
# VList = list(); for(q in returnMatrix[Split!="Train"]$fdateYQ){
#   relevantReturns = returnMatrix[fdateYQ < q, !c("fdateYQ", "Split")]
#   relevantReturns = Filter(function(x) !all(is.na(x)), relevantReturns)
#   covMat = cov(relevantReturns, use="pairwise.complete.obs")
#   #covMat = cov(returnMatrix[fdateYQ < q, !c("fdateYQ", "Split")], use="pairwise.complete.obs")
#   covMat = covMat[rowSums(is.na(covMat)) < ncol(covMat), colSums(is.na(covMat)) < nrow(covMat)]
#   VList = c(VList, list(covMat))
#   print(q)
# }; names(VList) = returnMatrix[Split!="Train"]$fdateYQ;
# #names(VList) = as.character(returnMatrix[Split!="Train"]$fdateYQ);
# View(VList$`2017.5`[10:30,10:30]); View(VList$`2022.5`[10:30,10:30]);


CovMatFunction = function(q){
  relevantReturns = returnMatrix[fdateYQ < q, !c("fdateYQ", "Split")]
  relevantReturns = Filter(function(x) !all(is.na(x)), relevantReturns)
  covMat = cov(relevantReturns, use="pairwise.complete.obs")
  #covMat = cov(returnMatrix[fdateYQ < q, !c("fdateYQ", "Split")], use="pairwise.complete.obs")
  covMat = covMat[rowSums(is.na(covMat)) < ncol(covMat), colSums(is.na(covMat)) < nrow(covMat)]
  covMat = covMat[rowSums(is.na(covMat)) < ncol(covMat)/5+1, colSums(is.na(covMat)) < nrow(covMat)/5+1]
  covMat = covMat[rowSums(is.na(covMat)) == 0, colSums(is.na(covMat)) == 0]
  covMat
}

# V=VList[[10]]
# View(V)
# colSums(is.na(V))
# rowSums(is.na(V))

#VListLessNA = lapply(VList, function(x) x[rowSums(is.na(x)) != ncol(x), colSums(is.na(x)) != nrow(x)])

# VListLessNA = VList[1:2]
# VListLessNA = VList$`2017.5`[rowSums(is.na(VList$`2017.5`)) < ncol(VList$`2017.5`), 
#                              colSums(is.na(VList$`2017.5`)) < nrow(VList$`2017.5`)]
# VListLessNA = VList$`2022.5`[rowSums(is.na(VList$`2022.5`)) < ncol(VList$`2022.5`), 
#                              colSums(is.na(VList$`2022.5`)) < nrow(VList$`2022.5`)]
# 
# lapply(VListLessNA, function(DT)
#   DT[,
#      which(unlist(lapply(DT,
#                          function(x)
#                            ! all(is.na(
#                              x
#                            ))))),
#      with = F])

# # apply(VList$`2017.5`, FUN = function(x) all(is.na(x)), MARGIN = 1)
# # apply(VList$`2017.5`, FUN = function(x) all(!is.na(x)), MARGIN = 1)
# 
# VListLessNA = lapply(VList, function(x) x[rowSums(is.na(x)) < ncol(x), colSums(is.na(x)) < nrow(x)])
# # VListLessNA = lapply(VList, function(x) x[apply(x, FUN = function(x) all(is.na(x)), MARGIN = 1), 
# #                                           apply(x, FUN = function(x) all(is.na(x)), MARGIN = 2)])
# VListLessNA = lapply(VList, function(x) x[rowSums(is.na(x)) < ncol(x)/5+1, colSums(is.na(x)) < nrow(x)/5+1])
# VListLessNA = lapply(VListLessNA, function(x) x[rowSums(is.na(x)) == 0, colSums(is.na(x)) == 0])
# # V=VListLessNA[[10]]
# # View(V)




# RwithTickers = firmPreds[gvkey %in% pooledForVariance$gvkey,]
# R = RwithTickers$V1

# RwithTickers = lapply(allPreds, function(x) x[gvkey %in% pooledForVariance$gvkey,])
# R = lapply(RwithTickers, function(x) x$V1)

R = list()
RwithTickers = list()
for(n in 1:length(allPredsList)){
  RwithTicker = allPredsList[[n]][allPredsList[[n]]$gvkey %in% colnames(VListLessNA[[n]]), ]
  R = c(R, list(RwithTicker$predictedReturn))
  RwithTickers = c(RwithTickers, list(RwithTicker))
}



# #RAndRF = list(R=R, rf0=rf0)
# 
# rfMatrix=list()
# for(n in 1:length(R)){
#   rfRep = rep(rf0[n], length(R[[n]]))
#   rfMatrix = c(rfMatrix, list(rfRep))
# }
# 
# rfMatr = do.call(cbind, rfMatrix)
# RMatr = do.call(cbind, R)
# 
# excessReturn = RMatr - rfMatr
# 
# 


excessReturn=list()
for(n in 1:length(rf0)){
  eR = R[[n]] - rf0[n]
  excessReturn = c(excessReturn, list(eR))
}



# length(V)
# sum(is.na(V))
# dim(V)
# isSymmetric(V)
# 
# #doubleChecking if PSD with a ready made package
# #matrixcalc::is.positive.semi.definite(V)
# 
# 
# V=VList[[10]]
# #packages for taking inverses of matrices
# inverse = Rfast::spdinv(V)
# inverse = mnormt::pd.solve(V)
# inverse = chol2inv(chol(V))
# inverse = solve(V)
# inverse = MASS::ginv(V)
# inverse = matlib::inv(V)
# View(V[1:10,1:10])
# 
# 
# length(inverse)
# sum(is.na(inverse))
# dim(inverse)
# isSymmetric(inverse)
# #View(inverse[1:10,1:10])
# 
# #Tangency portfolio
# numerator = inverse %*% (R-rf)
# denominator = rep(1, nrow(inverse)) %*% inverse %*% (R-rf)
# weights <- numerator / denominator[1]   #dividing the numerator by denominator gives us the weights
# sum(weights)
# 
# 
# portfolio = data.table("gvkey" = RwithTickers$gvkey, "date" = yearMonth, "weight" = c(weights))
# 
# 
# testYwithKey = pooled[,c("gvkey", "returns")]
# portfolioWithRealized = merge(portfolio, testYwithKey, by = "gvkey") 
# sum(portfolioWithRealized$weight)



#Invert
invList = lapply(VListLessNA, MASS::ginv)
for(n in 1:length(invList)){
  colnames(invList[[n]]) = colnames(VListLessNA[[n]])
  rownames(invList[[n]]) = rownames(VListLessNA[[n]])
}

#invList2 = lapply(VListLessNA, Rfast::spdinv)
VList=VListLessNA

#Equivalent solution with lists
weights=list()
for(n in 1:length(excessReturn)) {
  V = invList[[n]]
  numerator = V %*% excessReturn[[n]]
  denominator = rep(1, nrow(V)) %*% V %*% excessReturn[[n]]
  weight = numerator / denominator[1]
  weights = c(weights, list(weight))
}


optimalPortfolios = c()
for(n in 1:length(returnMatrix[Split=="Test"]$fdateYQ)) {
  q = returnMatrix[Split=="Test"]$fdateYQ[n]
  
  #V = invList[[n]]
  V = CovMatFunction(q); cat(q, "CovMat", "\n");
  VColNames = colnames(V); VRowNames = rownames(V);
  #invV = MASS::ginv(V); cat(q, "CovMat inverted", "\n"); #solve and ginv give the same results. I think
  invV = solve(V); cat(q, "CovMat inverted", "\n");
  colnames(invV) = VColNames; rownames(invV) = VRowNames
  
  ER = predsFactorsMerged[fdateYQ == q & gvkey %in% colnames(invV), c("NN1ER", "NN2ER", "NN3ER", "LinearER",
                                                                   "ZeroER", "FirmMeanER", "MeanER", 
                                                                   "gvkey", "fdateYQ")]
  
  #Something wrong with this observation. Only two observations in the MonthlyReturns data set
  ER = ER[!((gvkey==247670) & (fdateYQ==2020.75))] 
  
  V = V[rownames(V) %in% ER$gvkey, colnames(V) %in% ER$gvkey]
  invV = invV[rownames(invV) %in% ER$gvkey, colnames(invV) %in% ER$gvkey]
  
  #check correct order
  #as.character(eR$gvkey) == colnames(V)
  
  er = as.matrix(ER[,c("NN1ER", "NN2ER", "NN3ER", "LinearER",
                       "ZeroER", "FirmMeanER", "MeanER")])
  #Optimal tangency formula
  numerator = invV %*% er
  denominator = rep(1, nrow(invV)) %*% invV %*% er
  weight = sweep(numerator, 2, denominator, `/`)
  
  #Minimum variance formula
  numeratorMV = invV %*% rep(1, nrow(invV))
  denominatorMV = rep(1, nrow(invV)) %*% invV %*% rep(1, nrow(invV))
  weightMV = sweep(numeratorMV, 2, denominatorMV, `/`)
  
  weight = cbind(weight, weightMV)
  colnames(weight) = c("NN1Weights", "NN2Weights", "NN3Weights", "LinearWeights",
                       "ZeroWeights", "FirmMeanWeights", "MeanWeights", "MVWeights")
  
  ###Robin: check this:
  expectedPortfolioReturn = t(weight[,1]) %*%  er[,1]
  expectedPortfolioVariance = t(weight[,1]) %*% V %*% weight[,1]
  sharpe = expectedPortfolioReturn / sqrt(expectedPortfolioVariance)
  print(c(sharpe, expectedPortfolioReturn, sqrt(expectedPortfolioVariance)))
  # a = MASS::ginv(V)
  # b = solve(V)
  # ###
  for(p in 1:4){
    expectedPortfolioReturn = t(weight[,p]) %*%  er[,p]
  if(expectedPortfolioReturn<0){
    weight[,p] = -weight[,p]
  }}
  
  #ER = cbind(ER[, c("gvkey", "fdateYQ")], weight)
  ER = cbind(ER, weight)
  optimalPortfolios = rbind(optimalPortfolios, ER)
  cat(q, "Optimal portfolio calculated", "\n")
  
  print(colSums(optimalPortfolios[,c("NN1Weights", "NN2Weights", "NN3Weights", "LinearWeights",
                             "ZeroWeights", "FirmMeanWeights", "MeanWeights", "MVWeights")], na.rm = T))
  # print(colSums(optimalPortfolios[,c("NN1ER", "NN2ER", "NN3ER", "LinearER",
  #                      "ZeroER", "FirmMeanER", "MeanER")], na.rm = T))
  cat("\n", "\n")
}



#Check
#unlist(lapply(weights, sum))

optimalPortfolios[, list(sum(NN1Weights), 
                         sum(NN2Weights),
                         sum(NN3Weights)
                         ), by=fdateYQ]

optimalPortfolios[, list(sum(NN1ER * NN1Weights), 
                         sum(NN2ER * NN2Weights),
                         sum(NN3ER * NN3Weights)
                         ), by=fdateYQ]
#optimalPortfolios[, sum(NN1ER * NN1Weights) / sum(volas * NN1Weights), by=fdateYQ]


# for(n in allPortfolios){
#   name = paste0(n, "ExcpectedSharpe")
#   excessReturns = paste0(n, "ER")
#   volas = paste0(n, "Vola")
#   optimalPortfoliosWithRealized[, ":=" (name = sum(excessReturns * NN1Weights) / sum(volas * NN1Weights)), by=fdateYQ]
# }


#}################################################################
#sharpeMaximizingWeights()



# portfolios=list()
# for(n in 1:length(weights)) {
#   port = data.table("gvkey" = RwithTickers[[n]]$gvkey, "date" = cqtr[[n]], "weight" = c(weights[[n]]))
#   portfolios = c(portfolios, list(port))
# }


monthlyReturns = fread("Data/monthlyReturnData.csv")
#View(monthlyReturns[1:100,])
monthlyReturns$fdateYQ = paste(monthlyReturns$year, monthlyReturns$quarter, sep="Q")
monthlyReturns$fdateYQ = as.numeric(zoo::as.yearqtr(monthlyReturns$fdateYQ, format="%YQ%q"))
monthlyReturns$YM = paste(monthlyReturns$year, monthlyReturns$month, sep="M")
monthlyReturns$YM = zoo::as.yearmon(monthlyReturns$YM, format="%YM%m")

monthlyReturns = monthlyReturns[ , c("gvkey", "monthlyReturns", "fdateYQ", "year", "month", "YM")]


#b = merge(x=optimalPortfolios, y=monthlyReturns, by = c("gvkey", "fdateYQ"), all.x=T)
optimalPortfoliosWithRealized = merge(optimalPortfolios, monthlyReturns[fdateYQ>=min(optimalPortfolios$fdateYQ)], by = c("gvkey", "fdateYQ"), all=T)

sum(duplicated(optimalPortfoliosWithRealized, by = c("gvkey", "YM")))
sum(duplicated(optimalPortfoliosWithRealized, by = c("gvkey", "fdateYQ")))

sum(is.infinite(monthlyReturns$monthlyReturns))
optimalPortfoliosWithRealized = optimalPortfoliosWithRealized[!is.infinite(monthlyReturns),]

# b=optimalPortfoliosWithRealized[, length(NN1ER), by = c("gvkey", "fdateYQ")]
# bb = b[V1!=3]


optimalPortfolios[, list(sum(NN1Weights), 
                         sum(NN2Weights),
                         sum(NN3Weights)
                         ), by=fdateYQ]

optimalPortfoliosWithRealized[, list(sum(NN1Weights, na.rm=T), 
                         sum(NN2Weights, na.rm=T),
                         sum(NN3Weights, na.rm=T)
                         ), by=fdateYQ]

optimalPortfoliosWithRealized[, list(sum(NN1Weights, na.rm=T), 
                         sum(NN2Weights, na.rm=T),
                         sum(NN3Weights, na.rm=T)
                         ), by=YM]



#No duplicates! Great!
#anyDuplicated(monthlyReturns, incomparables=FALSE, fromLast=FALSE, by=c("gvkey", "fdateYQ", "month"))

# portfolioWithRealizedList = list()
# for(n in 1:length(weights)) {
#   portfolioWithRealized = merge(portfolios[[n]], monthlyReturns, by.x = c("gvkey", "date"), by.y = c("gvkey", "fdateYQ"))
#   portfolioWithRealizedList = c(portfolioWithRealizedList, list(portfolioWithRealized))
#   
#   sumN = sum(portfolioWithRealized$weight) #Should be approx. = 3
#   print(sumN)
# }




# #Pooled version. All inside one data.table
# pooledPortfolios=c()
# for(n in 1:length(weights)) {
#   port = data.table("gvkey" = RwithTickers[[n]]$gvkey, "fdateYQ" = cqtr[[n]], "weight" = c(weights[[n]]))
#   pooledPortfolios = rbind(pooledPortfolios, port)
# }
# 
# monthlyReturnsWithPortfolios = merge(monthlyReturns, pooledPortfolios, by = c("gvkey", "fdateYQ"))






# #testSet = testSetQuarters #From earlier in the script: TrainTestSplit_QuarterlySplit
# portfolioWithRealizedList = list()
# for(n in 1:length(weights)) {
#   
#   #cat("Model number", m)
#   set = testSet[[n]]
#   #set = Filter(is.numeric, set)
#   set = na.omit(set)
#   testYwithKey = set[, c("gvkey", "fdateYQ", "returns")]
#   portfolioWithRealized = merge(portfolios[[n]], testYwithKey, by = "gvkey")
#   portfolioWithRealizedList = c(portfolioWithRealizedList, list(portfolioWithRealized))
#   
#   sumN = sum(portfolioWithRealized$weight)
#   print(sumN)
# }


```

```{r SharpeMaximization using PortfolioAnalytics}
#LP, QP, or Numerical methods
#https://rdrr.io/cran/PortfolioAnalytics/src/demo/demo_max_Sharpe.R
require(PortfolioAnalytics)
require(data.table)
# q=2020.75

quarterlyReturnData = fread("Data/returnsData.csv", nThread = 4)
RAll = dcast.data.table(data=quarterlyReturnData, YearQuarter~gvkey, value.var="quarterlyReturns")
#View(RAll[1:10, 1:10])

# monthlyReturnData = fread("Data/monthlyReturnData.csv", nThread = 4)
# RAll = dcast.data.table(data=monthlyReturnData, YearQuarter~gvkey, value.var="monthlyReturns")


optimalWeights = function(q){
  R = as.data.frame(RAll)[-1, ]
  #R = returnMatrix[fdateYQ < q,]
  #rownames(R) = returnMatrix[fdateYQ < q,]$fdateYQ
  #R = subset(R, select = -c(fdateYQ, Split))
  R = R[R$YearQuarter < q & R$YearQuarter>=1985.75,]
  
  # #remove NA values?
  # forecastQ = RAll[RAll$YearQuarter == q, ]
  # forecastQ = unlist(forecastQ)
  # #forecastQ
  # 
  # forecastQ = na.omit(forecastQ)
  # forecastQ = names(forecastQ)
  # R = R[ , forecastQ]
  
  
  #keySubset = optimalPortfoliosWithRealized[fdateYQ==q, ]$gvkey
  #qPreds = allPreds[(fdateYQ==q) & (gvkey %in% keySubset), ]
  qPreds = allPreds[fdateYQ==q, ]
  R = R[ , colnames(R) %in% c(qPreds$gvkey, "YearQuarter")]
  
  
  R = R[rowSums(is.na(R)) < ncol(R), colSums(is.na(R)) < nrow(R)]
  # R = R[rowSums(is.na(R)) < ncol(R)/5+1, colSums(is.na(R)) < nrow(R)/5+1]
  R = R[rowSums(is.na(R)) < ncol(R)/5+1, ]
  R = R[, colSums(is.na(R)) < nrow(R)/5+1]
  # R = R[, colSums(is.na(R)) < 1]
  R = R[rowSums(is.na(R)) == 0, colSums(is.na(R)) == 0]
  
  rownames(R) = R$YearQuarter
  R = zoo::zoo(subset(R, select = -c(YearQuarter)), order.by=zoo::as.yearqtr(R$YearQuarter, ))
  #R = xts(subset(R, select = -c(YearQuarter)), order.by=zoo::as.yearqtr(R$YearQuarter))
  #R = subset(R, select = -c(YearQuarter))
  
  #R = as.matrix(R)
  #R = as.ts(R)
  #View(R[, 1:100])
  
  
  
  
  # View(RAll[, 1:100])
  #View(R[, 1:100])
  #R = R[gvkey %in% allPreds$gvkey]
  #R = na.omit(R)
  #R = R[, 1:100]
  finalR = R
  #finalR = R[, 1:70]
  finalR = R[, 1:20]
  #View(finalR)
  
  assets = names(finalR)
  search_size = 1000
  #search_size = 100
  
  init.portf <- portfolio.spec(assets=assets)
  init.portf <- add.constraint(portfolio=init.portf, type="full_investment")
  init.portf <- add.objective(portfolio=init.portf, type="return", name="mean") #change this objective to expected returns
  init.portf <- add.objective(portfolio=init.portf, type="risk", name="StdDev")
  init.portf.lo <- add.constraint(portfolio=init.portf, type="long_only")
  
  init.portf.rp = init.portf
  init.portf.rp$constraints[[1]]$min_sum=0.99
  init.portf.rp$constraints[[1]]$max_sum=1.01
  
  
  init.portf.lo.rp = init.portf.lo
  init.portf.lo.rp$constraints[[1]]$min_sum=0.99
  init.portf.lo.rp$constraints[[1]]$max_sum=1.01
  
  
  # qPreds = allPreds[fdateYQ==q, ]
  # finalR = R[ , qPreds$gvkey]
  
  my.covariance.matrix = cov(finalR)
  
  expected.returns = qPreds[gvkey %in% colnames(finalR), ]
  rownames(expected.returns) = expected.returns$gvkey
  #View(as.matrix(expected.returns))
  #View(my.covariance.matrix)
  
  
  num_assets = ncol(finalR)
  momentargs = list()
  momentargs$sigma = my.covariance.matrix
  # momentargs$m3 = matrix(0, nrow = num_assets, ncol = num_assets ^ 2)
  # momentargs$m4 = matrix(0, nrow = num_assets, ncol = num_assets ^ 3)
  momentargs$m3 = 0
  momentargs$m4 = 0
  
  
  res = c()
  for(n in c("BestModelPred", "SecondModelPred", "ThirdModelPred", "LinearModelPred")){
  momentargs$mu = expected.returns[ , n, with=F][[1]]
  maxSR.lo.RP.NN <- optimize.portfolio(R=finalR, portfolio=init.portf.lo.rp, momentargs = momentargs,
                                   optimize_method="random",
                                   maxSR=TRUE, trace=TRUE, search_size = search_size)
  maxSR.lo.RP.NN
  
  # maxSR.lo.ROI.NN <- optimize.portfolio(R=finalR, portfolio=init.portf.lo, momentargs = momentargs,
  #                                  optimize_method="ROI",
  #                                  maxSR=TRUE, trace=TRUE, search_size = search_size)
  # maxSR.lo.ROI.NN
  #chart.Weights(maxSR.lo.RP.NN)
  expectedSharpe = maxSR.lo.RP.NN$objective_measures$mean / sqrt(maxSR.lo.RP.NN$objective_measures$StdDev)
  print(expectedSharpe)
  # expectedSharpe = maxSR.lo.ROI.NN$objective_measures$mean / sqrt(maxSR.lo.ROI.NN$objective_measures$StdDev)
  # print(expectedSharpe)
  #print(sum(c(maxSR.lo.RP.NN$weights)))
  
  res = cbind(res, maxSR.lo.RP.NN$weights)
  }
  
  colnames(res) = c("NN1RPWeights", "NN2RPWeights", "NN3RPWeights", "LinearRPWeights")
  resWithKeys = cbind(res, 
        "gvkey"=as.numeric(names(maxSR.lo.RP.NN$weights)), 
        "fdateYQ"=rep(q, length(maxSR.lo.RP.NN$weights)))
  
  print(colSums(resWithKeys))
  resWithKeys
}

optimalPortfoliosFromPortfolioAnalytics = c()
system.time(
for(n in 1:length(returnMatrix[Split=="Test"]$fdateYQ)) {
  q = returnMatrix[Split=="Test"]$fdateYQ[n];
  print(q)
  newQWeights = optimalWeights(q)
  #optimalPortfoliosFromPortfolioAnalytics = rbind(optimalPortfoliosFromPortfolioAnalytics, c(newQWeights, "fdateYQ"=q))
  optimalPortfoliosFromPortfolioAnalytics = rbind(optimalPortfoliosFromPortfolioAnalytics, newQWeights)
}
)



optimalPortfoliosFromPortfolioAnalytics = as.data.table(optimalPortfoliosFromPortfolioAnalytics)
colSums(optimalPortfoliosFromPortfolioAnalytics)

optimalPortfoliosFromPortfolioAnalytics[, list(
  sum(NN1RPWeights, na.rm = T), 
  sum(NN2RPWeights, na.rm = T), 
  sum(NN3RPWeights, na.rm = T)
  ), by = fdateYQ]


optimalPortfoliosWithRealized = merge(optimalPortfoliosWithRealized, 
                                      optimalPortfoliosFromPortfolioAnalytics, 
                                      by = c("gvkey", "fdateYQ"), all=T)

#bb = optimalPortfoliosWithRealized[ , length(NN1RPWeights), by = c("gvkey", "fdateYQ")]



optimalPortfoliosWithRealized[, list(
  sum(NN1Weights, na.rm = T),
  sum(NN2Weights, na.rm = T),
  sum(NN3Weights, na.rm = T),
  sum(NN1RPWeights, na.rm = T),
  sum(NN2RPWeights, na.rm = T),
  sum(NN3RPWeights, na.rm = T)
), by = fdateYQ]

optimalPortfoliosWithRealized[, list(
  sum(NN1Weights, na.rm = T),
  sum(NN2Weights, na.rm = T),
  sum(NN3Weights, na.rm = T),
  sum(NN1RPWeights, na.rm = T),
  sum(NN2RPWeights, na.rm = T),
  sum(NN3RPWeights, na.rm = T)
), by = YM]


# init.portf
# init.portf.lo
# init.portf.rp
# init.portf.lo.rp
# 
# 
# maxSR.ROI <- optimize.portfolio(R=finalR, portfolio=init.portf, 
#                                    optimize_method="ROI", 
#                                    maxSR=TRUE, trace=TRUE, search_size = search_size)
# maxSR.ROI
# chart.Weights(maxSR.ROI)
# 
# 
# maxSR.lo.ROI <- optimize.portfolio(R=finalR, portfolio=init.portf.lo, 
#                                    optimize_method="ROI", 
#                                    maxSR=TRUE, trace=TRUE, search_size = search_size)
# maxSR.lo.ROI
# chart.Weights(maxSR.lo.ROI)
# #chart.EfficientFrontier(maxSR.lo.ROI, match.col = "StdDev", n.portfolios = 5)#, xlim = c(0, 30), ylim = c(0, 10))
# 
# 
# 
# 
# 
# 
# 
# maxSR.ROI.NN <- optimize.portfolio(R=finalR, portfolio=init.portf, momentargs = momentargs,
#                                    optimize_method="ROI", 
#                                    maxSR=TRUE, trace=TRUE, search_size = search_size)
# maxSR.ROI.NN
# chart.Weights(maxSR.ROI.NN)
# 
# maxSR.lo.ROI.NN <- optimize.portfolio(R=finalR, portfolio=init.portf.lo, momentargs = momentargs,
#                                    optimize_method="ROI", 
#                                    maxSR=TRUE, trace=TRUE, search_size = search_size)
# maxSR.lo.ROI.NN
# chart.Weights(maxSR.lo.ROI.NN)
# 
# 
# 
# 
# 
# maxSR.RP.NN <- optimize.portfolio(R=finalR, portfolio=init.portf.rp, momentargs = momentargs,
#                                    optimize_method="random",
#                                    maxSR=TRUE, trace=TRUE, search_size = 200)
# maxSR.RP.NN
# chart.Weights(maxSR.RP.NN)
# 
# 
# maxSR.lo.RP.NN <- optimize.portfolio(R=finalR, portfolio=init.portf.lo.rp, momentargs = momentargs,
#                                    optimize_method="random",
#                                    maxSR=TRUE, trace=TRUE, search_size = 200)
# maxSR.lo.RP.NN
# chart.Weights(maxSR.lo.RP.NN)
# 
# 
# 
# 
# optimalPortfoliosWithRealized
# maxSR.lo.RP.NN$weights
# 


```


```{r NaivePortfolios}

# #Equal-weighted portfolio: 1/n
# portfolioWithRealizedList = lapply(portfolioWithRealizedList, 
#                                    FUN= function(x) x[,":=" ("equalWeight"=1/length(gvkey)), by = fdateYQ])
# 
# monthlyReturnsWithPortfolios = monthlyReturnsWithPortfolios[, ":=" ("equalWeights"= 1/length(gvkey)), by = fdateYQ]

optimalPortfoliosWithRealized = optimalPortfoliosWithRealized[, ":=" ("EqualWeights"= 1/length(gvkey)), by = YM]

optimalPortfoliosWithRealized[, sum(EqualWeights), by = YM]

#Value-weighted (Global) portfolio: MktCap / sum(AllMktCaps)

#OLS model: Above script

#Best model 1, 2, and 3: Above script

#NaiveZeroModel: Above script with R = 0

#NaiveMeanModel: Above script with R = mean. It's the same as the minimum variance portfolio

#NaiveFirmMeanModel: Above script with R = mean_n



```


```{r Classification porfolios}


#targets = c("['2Classes']", "['4Classes']", "['6Classes']", "['8Classes']", "['10Classes']")
# allModels = c()
# for(x in targets){
#   models = c()
#   for(n in 1:3){
#     path = sprintf("Results/%s/Ordinary/SavedKerasModels/BestKerasModel%s", x, n)
#     fs::dir_tree(path)
#     model <- keras::load_model_tf(path, custom_objects = list("MDA": MDA))
#     models = c(models, model)
#   }
#   StandardLinearModel <- keras::load_model_tf(sprintf("Results/%s/Ordinary/SavedKerasModels/KerasLinearModel", x))
#   models = c(models, model)
#   allModels = cbind(allModels, models)
#   }
# colnames(models) = target



require(data.table)
targets = c("['2Classes']", "['4Classes']", "['6Classes']", "['8Classes']", "['10Classes']")
dt = NULL
for(x in targets){
  path = sprintf("Results/%s/Ordinary/ClassificationPredictions.csv", x)
  newDt = fread(path)[, !c("Model3", "Model4", "Model5"), with=F]
  
  
  for(n in c("Model1", "Model2", "Model3", "Model4", "Model5", "LogitModel","Ensemble")){
    #colnames(newDt[, n, with=F]) = paste0(n, x)
    setnames(newDt, n, paste0(n, x), skip_absent=T)
  }
  dt = c(dt, list(newDt))
  #dt = merge(x = newDt, y = dt, by = c("gvkey", "fdateYQ"), all.x = TRUE)
}

dt = Reduce(function(...) merge(..., by= c("gvkey", "fdateYQ")), dt)
#dt = do.call("cbind", dt)


# df$Model1
# colnames(df)
# df["Model1"]

for(n in colnames(dt)){
  dt[dt[[n]]==1, paste0(n, "Weights") := 1/length(gvkey), by=fdateYQ]
  dt[dt[[n]]==-1, paste0(n, "Weights") := -1/length(gvkey), by=fdateYQ]
}

optimalPortfoliosWithRealized = merge(optimalPortfoliosWithRealized, dt, on = c("gvkey", "fdateYQ"), all = T)


exportN = 12
fwrite(optimalPortfoliosWithRealized, file = paste0("Data/OptimalWeights", exportN, ".csv"))

```


```{r Turnover}


#optimalPortfoliosWithRealized = fread(file = "Data/OptimalWeights.csv")


#Specify general model names (suffixes)
regressionNames = c("NN1", "NN2", "NN3", "Linear", "NN1RP", "NN2RP", "NN3RP", "LinearRP", "MV", "FirmMean", "Equal")
#regressionNames = c("NN1RP", "NN2RP", "NN3RP", "LinearRP", "MV", "FirmMean", "Equal")

# classificationNames = paste0(
#         c("Model1", "Model2", "Ensemble"),
#   c("['2Classes']", "['4Classes']", "['6Classes']", "['8Classes']", "['10Classes']")
#   )

cartesianProduct = expand.grid(c("Model1", "Model2", "LogitModel", "Ensemble"), 
            c("['2Classes']", "['4Classes']", "['6Classes']", "['8Classes']", "['10Classes']"))
classificationNames = apply(cartesianProduct, MARGIN=1, function(x) paste0(x[1], x[2]))

# classificationNames = c("Model1['2Classes']", "Model2['2Classes']", "Ensemble['2Classes']", "Model1['4Classes']", "Model2['4Classes']", "Ensemble['4Classes']", "Model1['6Classes']", "Model2['6Classes']", "Ensemble['6Classes']", "Model1['8Classes']", "Model2['8Classes']", "Ensemble['8Classes']", "Model1['10Classes']", "Model2['10Classes']", "Ensemble['10Classes']")

allPortfolios = c(regressionNames, classificationNames)

shortingNames = c(classificationNames, "NN1", "NN2", "NN3", "Linear", "MV", "FirmMean")
nonFullInvestmentNames = c(classificationNames, "NN1", "NN2", "NN3", "Linear")


for(n in allPortfolios){
  w = optimalPortfoliosWithRealized[[paste0(n, "Weights")]]
  optimalPortfoliosWithRealized[, paste0(n, "Return") :=  w * monthlyReturns]
  optimalPortfoliosWithRealized[, paste0(n, "PortfolioReturn") :=  sum(get(paste0(n, "Return")), na.rm = T), by = c("YM")]
  
  optimalPortfoliosWithRealized[, paste0(n, "MktAdjWeight") :=  w * (1+monthlyReturns)]
  #optimalPortfoliosWithRealized[, sum(get(paste0(n, "Weights")), na.rm=T), by = c("YM")]
}


na.omit(optimalPortfoliosWithRealized[, c(paste0(n, "Weights"), paste0(n, "MktAdjWeight")), with=F])

#Turnover
weightsMatrices = lapply(paste0(allPortfolios, "Weights"),
                        FUN = function(x) dcast(optimalPortfoliosWithRealized, formula=YM~gvkey, value.var = x))

weightsMatricesMktAdj = lapply(paste0(allPortfolios, "MktAdjWeight"),
                        FUN = function(x) dcast(optimalPortfoliosWithRealized, formula=YM~gvkey, value.var = x))

#weightsMatrices = dcast(optimalPortfoliosWithRealized, formula=fdateYQ~gvkey, value.var = paste0(allPortfolios, "Weights"))

#lapply(weightsMatrices, FUN=function(x) apply(x, MARGIN=2, FUN=sd))

turnoverMktAdj = c()
for(i in 1:length(weightsMatrices)){
  if(!(allPortfolios[i] %in% nonFullInvestmentNames)) {
    scaledMktWeights = weightsMatricesMktAdj[[i]][ , !c("YM")] / rowSums(weightsMatricesMktAdj[[i]][ , !c("YM")], na.rm = T)
  }
  else scaledMktWeights = weightsMatricesMktAdj[[i]][ , !c("YM")]
  difference = weightsMatrices[[i]][ , !c("YM")] - scaledMktWeights
  difference = unlist(difference)
  difference = na.omit(difference)
  turnover_i = sum(abs(difference), na.rm=T) / length(difference)
  turnoverMktAdj = c(turnoverMktAdj, turnover_i)
  print(c(allPortfolios[i], signif(turnover_i, 3)))
}
names(turnoverMktAdj) = allPortfolios

weightsMatricesNumeric = lapply(weightsMatrices, FUN=function(x) apply(x[rownames(x) != "YM"], MARGIN=1, FUN= as.numeric ))
turnover = unlist(lapply(weightsMatricesNumeric, 
                         FUN= function(x) sum(abs(diff(x)), na.rm = T) / nrow(x))); names(turnover) = allPortfoliosCleaned
as.matrix(round(turnover, 4))



```


```{r AbnormalReturns}




#optimalPortfoliosWithRealized[, lapply(list(get(paste0(allPortfolios, "Weights"))), sum, na.rm=T), by = c("YM")]


for(n in allPortfolios){
    optimalPortfoliosWithRealized[, paste0(n, "PortfolioWeights") :=  sum(get(paste0(n, "Weights")), na.rm = T), by = c("YM")]
}


as.matrix(colSums(optimalPortfoliosWithRealized[, paste0(allPortfolios, "Weights"), with=F], na.rm = T))
optimalPortfoliosWithRealized[, colSums(as.matrix(get(paste0(allPortfolios, "Weights"))), na.rm = T), with=F]

monthlyPortfolioWeights = optimalPortfoliosWithRealized[, list(
  sum(NN1Weights, na.rm=T),
  sum(NN2Weights, na.rm=T),
  sum(NN3Weights, na.rm=T),
  sum(NN1RPWeights, na.rm=T),
  sum(NN2RPWeights, na.rm=T),
  sum(NN3RPWeights, na.rm=T),
  sum(LinearRPWeights, na.rm=T),
  sum(MVWeights, na.rm=T),
  sum(MeanWeights, na.rm=T),
  sum(FirmMeanWeights, na.rm=T),
  sum(EqualWeights)
), by = c("YM")]

optimalPortfoliosWithRealized[, list(
  sum(NN1Weights, na.rm=T),
  sum(NN2Weights, na.rm=T),
  sum(NN3Weights, na.rm=T),
  sum(NN1RPWeights, na.rm=T),
  sum(NN2RPWeights, na.rm=T),
  sum(NN3RPWeights, na.rm=T),
  sum(MeanWeights, na.rm=T),
  sum(FirmMeanWeights, na.rm=T),
  sum(EqualWeights, na.rm=T)
), by = c("fdateYQ")]


realizedPortfolios = unique(
  optimalPortfoliosWithRealized[, c("YM", 
                                    paste0(allPortfolios, "PortfolioReturn"), 
                                    paste0(allPortfolios, "PortfolioWeights")), 
                                with=F], by = c("YM"))
realizedPortfolios[as.numeric(YM)%%1==0, "YM" := YM + 1]


monthlyFactors=fread("FactorData/allMonthlyFactors.csv")
monthlyFactors$YM = zoo::as.yearmon(as.character(monthlyFactors$V1), format = "%Y%m")

factorsWithReturns = merge(realizedPortfolios, monthlyFactors, by.x = "YM", by.y="YM", all = TRUE)


for(n in allPortfolios){
  if(n %in% c("NN1", "NN2", "NN3", "Linear")) {
    factorsWithReturns[get(paste0(n, "PortfolioWeights")) < 0, 
                       paste0(n, "PortfolioReturnCostAdj") :=  get(paste0(n, "PortfolioReturn")) + 0.5 * RF.DM - 0.01 * turnoverMktAdj[n]*100]
    factorsWithReturns[get(paste0(n, "PortfolioWeights")) >= 0, 
                       paste0(n, "PortfolioReturnCostAdj") :=  get(paste0(n, "PortfolioReturn")) - 0.01 * turnoverMktAdj[n]*100]
  }
  else if(n %in% classificationNames) {
    factorsWithReturns[, paste0(n, "PortfolioReturnCostAdj") :=  get(paste0(n, "PortfolioReturn")) * (1/1.5) - 0.01 * turnoverMktAdj[n]*100]
    }
  else {
    factorsWithReturns[, paste0(n, "PortfolioReturnCostAdj") :=  get(paste0(n, "PortfolioReturn")) - 0.01 * turnoverMktAdj[n]*100]
    }
  
  factorsWithReturns[, paste0(n, "ER") :=  get(paste0(n, "PortfolioReturn")) - RF.DM, by = c("YM")]
  factorsWithReturns[, paste0(n, "ERCostAdj") :=  get(paste0(n, "PortfolioReturnCostAdj")) - RF.DM, by = c("YM")]
}



FF6Factors = colnames(monthlyFactors[, !c("V1", "YM", "RF.DM", "RF.EM"), with =F])
FF6Factors = gsub("-RF", "", FF6Factors)
FF6Factors = gsub("\\.", "_", FF6Factors)

colnames(factorsWithReturns) = gsub("-RF", "", colnames(factorsWithReturns))
colnames(factorsWithReturns) = gsub("\\.", "_", colnames(factorsWithReturns))
colnames(factorsWithReturns) = gsub("\\['", "_", colnames(factorsWithReturns))
colnames(factorsWithReturns) = gsub("\\']", "", colnames(factorsWithReturns))

allPortfoliosCleaned = gsub("\\['", "_", allPortfolios)
allPortfoliosCleaned = gsub("\\']", "", allPortfoliosCleaned)


createModels = function(ER_String) {
  formulas = list()
  models = list()
  for (n in allPortfoliosCleaned) {
    f = paste(paste0(n, ER), "~", paste(FF6Factors, collapse = " + "))
    formulas = c(formulas, list(f))
    models = c(models, list(lm(f, data = factorsWithReturns)))
  }
  names(formulas) = paste0(allPortfoliosCleaned, "F")
  names(models) = allPortfoliosCleaned
  
  return(models)
}

models = createModels("ER")
modelsAdj = createModels("ERCostAdj")

# # realizedPortfolios = lapply(portfolioWithRealizedList, function(x) x[, c("weight", "monthlyReturns", "YM")])
# # realizedPortfolios = lapply(realizedPortfolios, function(x) x[, "portfolioReturn" := x$weight * x$monthlyReturns])
# 
# #Using lists
# #realizedPortfolios = lapply(portfolioWithRealizedList, function(x) x$weight * x$returns)
# realizedPortfolios = lapply(portfolioWithRealizedList, function(x) x$weight * x$monthlyReturns)
# realizedPortfolios = lapply(portfolioWithRealizedList, function(x) x$weight * x$monthlyReturns)
# portfolioReturn = lapply(realizedPortfolios, function(x) sum(x))
# 
# 
# dateList = lapply(portfolioWithRealizedList, function(x) x$YM)
# portfolioPerformance = cbind("Excess Return"=unlist(portfolioReturn), "date"=unlist(dateList))
# #factorsWithReturns = merge(portfolioPerformance, quarterlyFactors, by.x = "date", by.y="YQ", all = TRUE)
# factorsWithReturns = merge(portfolioPerformance, monthlyFactors, by.x = "YM", by.y="YM", all = TRUE)
# 
# # dateList = lapply(portfolioWithRealizedList, function(x) max(x$date))
# # portfolioPerformance = cbind("Excess Return"=unlist(portfolioReturn), "date"=unlist(dateList))
# # #factorsWithReturns = merge(portfolioPerformance, quarterlyFactors, by.x = "date", by.y="YQ", all = TRUE)
# # factorsWithReturns = merge(portfolioPerformance, monthlyFactors, by.x = "date", by.y="V1", all = TRUE)
# 
# 
# f = `Excess Return` ~ Mkt+SMB+HML+RMW+CMA+WML
# FamaFrenchSix = lm(f, data=factorsWithReturns)
# summary(FamaFrenchSix)

```




```{r FamaFrenchReporting}


#winsorize
#winData = Winsorize(data, probs = c(0.01, 0.99), na.rm=T)

diagnosticsFunction = function(modelList){
  residuals = lapply(modelList, residuals)
  JB = lapply(residuals, moments::jarque.test)
  BP = lapply(modelList, lmtest::bptest, data=factorsWithReturns)
  BG = lapply(modelList, lmtest::bgtest, order=7, data=factorsWithReturns)
  
  VIF = as.data.frame(lapply(modelList, function(x) data.frame(round(car::vif(x), 2))))
  colnames(VIF)=paste0(allPortfoliosCleaned, "VIF")
  VIF1 = VIF[1]
  colnames(VIF1) = "VIF"
  write.csv(VIF1, paste0("Results/VIF", exportN, ".csv"))
  
  diagnostics = list(c("Test" = "Jarque-Bera (p-value)", unlist(lapply(JB, function(x) round(x$p.value, 2)))),
                     c("Test" = "Breusch-Pagan (p-value)", unlist(lapply(BP, function(x) round(x$p.value[[1]], 2)))),
                     c("Test" = "Breusch-Godfrey (p-value)", unlist(lapply(BG, function(x) round(x$p.value, 2)))))
  
  diagnosticsStatValues = list(c("Test" = "Jarque-Bera", unlist(lapply(JB, function(x) round(x$statistic[[1]], 2)))),
                     c("Test" = "Breusch-Pagan", unlist(lapply(BP, function(x) round(x$statistic[[1]], 2)))),
                     c("Test" = "Breusch-Godfrey", unlist(lapply(BG, function(x) round(x$statistic[[1]], 2)))))
  diagnostics
}

diagnostics = diagnosticsFunction(models)
diagnosticsAdj = diagnosticsFunction(modelsAdj)

# #Unit root for panel data
# finalNumericData = Filter(is.numeric, YXfactor[, -c("gvkey", "fdateq")])
# #sum(is.na(finalNumericData))
# finalNumericData = na.omit(finalNumericData)
# compustatStationarity = plm::purtest(finalNumericData)




# #Unit root
# fUnitRoots::adfTest(factorsWithReturns$excessReturn, lags = 1, type = c("nc"), title = NULL, description = NULL)


# #Slumpmässig eller fasteffekt (Hausman)
# phtest(firmRandom,firmFixed)
# summary(firmFixedTwo) #bättre
# 
# #Slump eller pooled
# plmtest(factorsWithReturns, type="bp")
# 
# #Fast eller pooled
# plm::pFtest(firmFixedTwo, firmPooledTwo)
# lmtest::waldtest(firmFixedTwo, firmPooledTwo)
# plm::pwaldtest(firmFixed, firmPooled)



#turnoverForReporting = list(c("Test"= "Turnover (%)", signif(turnoverMktAdj*100, 2)))
turnoverForReporting = list(c("Test"= "Turnover (%)", round(turnoverMktAdj*100, 2)))

allAddedLines = c(diagnostics, turnoverForReporting)

#Export all
namesForStargazer = gsub("_", ":", names(models))
namesForStargazer = gsub("Classes", "C", namesForStargazer)
namesForStargazer = gsub("LogitModel", "Logit", namesForStargazer)
namesForStargazer = gsub("Model", "NN", namesForStargazer)
namesForStargazer = gsub("Ensemble", "E", namesForStargazer)
#namesForStargazer = gsub("NN", "M", namesForStargazer)
namesForStargazer = gsub("RP", ":RPLO", namesForStargazer)

stargazer::stargazer(models, 
                     dep.var.labels	= namesForStargazer, 
                     dep.var.caption= "Dependent variable: Excess return", 
                    title ="Fama-French six-factor model: All models",
                     report=("vc*t"),
                     add.lines = allAddedLines,
                     digits = 2, no.space = T,
                     type="text", out = paste0("Results/AllModels", exportN, ".html")
                     )


#Export regression
regModels = models[regressionNames]
regDiagnostics = lapply(diagnostics, FUN=function(x) x[c("Test", regressionNames)])
regTurnover = lapply(turnoverForReporting, FUN=function(x) x[c("Test", regressionNames)])
regNamesForStargazer = gsub("_", ":", names(regModels))
#regNamesForStargazer = gsub("NN", "M", regNamesForStargazer)
regNamesForStargazer = gsub("RP", ":RPLO", regNamesForStargazer)

regPredictionAccuracy = predictionAccuracySummary[c("OOS_R2_Zero", "OOS_R2_Mean", "OOS_R2_Linear", "OOS_RA_Zero", "OOS_RA_Mean", "OOS_RA_Linear"), 
                                                  c("BestModelPred", "SecondModelPred", "ThirdModelPred", "LinearModelPred")]
regPredictionAccuracy = round(regPredictionAccuracy*100, 2)

regPredictionAccuracy = cbind("Test"=c("OOS-R2-Zero (%)", "OOS-R2-Mean (%)", "OOS-R2-Linear (%)", 
                                       "OOS-RA-Zero (%)", "OOS-RA-Mean (%)", "OOS-RA-Linear (%)"), 
                              regPredictionAccuracy)
regDMForStargazer = apply(DMPValues, MARGIN=2, as.numeric)
regDMForStargazer = round(regDMForStargazer, 2)
regDMForStargazer = cbind(
  "Test" = c(
    "Diebold-Mariano-Zero (p-value)",
    "Diebold-Mariano-Mean (p-value)",
    "Diebold-Mariano-Linear (p-value)",
    "Diebold-Mariano-A-Zero (p-value)",
    "Diebold-Mariano-A-Mean (p-value)",
    "Diebold-Mariano-A-Linear (p-value)"
  ),
  regDMForStargazer
)
                          
regPredictionAccuracy = as.list(as.data.frame(t(regPredictionAccuracy)))
regDMForStargazer = as.list(as.data.frame(t(regDMForStargazer)))
regAddedLines = c(regDiagnostics, regTurnover, regPredictionAccuracy, regDMForStargazer)

stargazer::stargazer(regModels,
                     dep.var.labels = regNamesForStargazer, 
                     dep.var.caption= "Dependent variable: Excess return", 
                     title ="Fama-French six-factor model: Regression models and Naive models",
                     #column.labels= c("Trained Models", "Benchmarks"), column.separate = c(length(regNamesForStargazer)-3, 3),
                     report=("vc*t"),
                     add.lines = regAddedLines,
                     digits = 2, no.space = T,
                     type="text", out = paste0("Results/RegressionModels", exportN, ".html")
                     )


#Export classification
classModels = models[setdiff(names(models), regressionNames)]
classDiagnostics = lapply(diagnostics, FUN=function(x) x[c("Test", setdiff(names(models), regressionNames))])
classTurnover = lapply(turnoverForReporting, FUN=function(x) x[c("Test", setdiff(names(models), regressionNames))])

classNamesForStargazer = gsub("_", ":", names(classModels))
classNamesForStargazer = gsub("Classes", "C", classNamesForStargazer)
classNamesForStargazer = gsub("LogitModel", "Logit", classNamesForStargazer)
classNamesForStargazer = gsub("Model", "NN", classNamesForStargazer)
classNamesForStargazer = gsub("Ensemble", "E", classNamesForStargazer)

classAddedlines= c(classDiagnostics, classTurnover)

stargazer::stargazer(classModels,
                     dep.var.labels	 = classNamesForStargazer, 
                     dep.var.caption= "Dependent variable: Excess return", 
                     title ="Fama-French six-factor model: Classification models",
                     report=("vc*t"),
                     add.lines = classAddedlines,
                     digits = 2, 
                     column.sep.width = "1pt", no.space = T,
                     type="text", out = paste0("Results/ClassificationModels", exportN, ".html")
                     )



#robust errors: HAC?
#robustErrors = sandwich::vcovHC(models[[1]], method="arellano")
robustErrors = lapply(models, sandwich::vcovHAC)
robustErrorsCA = lapply(modelsAdj, sandwich::vcovHAC)


#Export with Robust errors
stargazer::stargazer(models,                     
                     dep.var.labels	= namesForStargazer, 
                     dep.var.caption= "Dependent variable: Excess return", 
                     title ="Fama-French six-factor model: All models with robust standard errors",
                     report=("vc*t"), 
                     add.lines = turnoverForReporting,
                     digits = 2, #digits.extra =1,
                     se  = robustErrors,
                     type="text", out = paste0("Results/AllModelsWithRobustErrors", exportN, ".html")
                     )




#Export cost adjusted
stargazer::stargazer(modelsAdj,                     
                     dep.var.labels	= namesForStargazer, 
                     dep.var.caption= "Dependent variable: Excess return", 
                     title ="Fama-French six-factor model: Cost adjusted models with robust standard errors",
                     report=("vc*t"),
                     add.lines = turnoverForReporting,
                     digits = 2, #digits.extra =1,
                     se  = robustErrorsCA,
                     type="text", out = paste0("Results/CostAdjustedModelsWithRobustErrors", exportN, ".html")
                     )



```



```{r RobustTesting}

#FWER (Bonferroni and Holm) and FDR


#Bonferroni correction with 0.16 %
bonferroniThreshold = qt(p=0.0016/2, df=9, lower.tail=TRUE)
qt(p=0.0016/2 * 2, df=9, lower.tail=TRUE)
bonferroniThreshold

pt(bonferroniThreshold, df=9, lower.tail=TRUE)*31




alphaPValues = unlist(lapply(models, FUN= function(x) summary(x)$coefficients["(Intercept)",4]))
robustCoefs = lapply(models, FUN=function(x) lmtest::coeftest(x, vcov = sandwich::vcovHAC(x)))
robustAlphaPValues = unlist(lapply(robustCoefs, FUN= function(x) x["(Intercept)",4]))

#Regular
bonferroniPValues= p.adjust(p=alphaPValues, method="bonferroni")
holmPValues = p.adjust(p=alphaPValues, method="holm")
FDRPValues = p.adjust(p=alphaPValues, method="BH")


#Robust
bonferroniPValuesRobust = p.adjust(p=robustAlphaPValues, method="bonferroni")
holmPValuesRobust = p.adjust(p=robustAlphaPValues, method="holm")
FDRPValuesRobust = p.adjust(p=robustAlphaPValues, method="BH")

#bonferroniForExport = list(c("Test" = "Bonferroni (p-value)", round(bonferroniPValuesRobust, 3)))
holmForExport = list(c("Test" = "Holm (p-value)", round(holmPValuesRobust, 3)))
FDRForExport = list(c("Test" = "BH-FDR (p-value)", round(FDRPValuesRobust, 3)))





#F-Test
#coef(models[[1]])["(Intercept)"]
#alphaHypotheses = car::linearHypothesis(models[[1]], c("(Intercept) = 0"))



#Export with Robust errors
stargazer::stargazer(models,                     
                     dep.var.labels	= namesForStargazer, 
                     dep.var.caption= "Dependent variable: Excess return", 
                     title ="Fama-French six-factor model: All models with robust standard errors",
                     report=("vc*t"),
                     add.lines = c(turnoverForReporting, holmForExport, FDRForExport),
                     digits = 2, #digits.extra =1,
                     se  = robustErrors,
                     type="text", out = paste0("Results/AllModelsWithRobustErrorsAndCorrections", exportN, ".html")
                     )




#Robust and adjusted
alphaPValuesAdj = unlist(lapply(modelsAdj, FUN= function(x) summary(x)$coefficients["(Intercept)",4]))
robustCoefsAdj = lapply(modelsAdj, FUN=function(x) lmtest::coeftest(x, vcov = sandwich::vcovHAC(x)))
robustAlphaPValuesAdj = unlist(lapply(robustCoefsAdj, FUN= function(x) x["(Intercept)",4]))

bonferroniPValuesRobustAdj = p.adjust(p=robustAlphaPValuesAdj, method="bonferroni")
holmPValuesRobustAdj = p.adjust(p=robustAlphaPValuesAdj, method="holm")
FDRPValuesRobustAdj = p.adjust(p=robustAlphaPValuesAdj, method="BH")

#bonferroniForExport = list(c("Test" = "Bonferroni (p-value)", round(bonferroniPValuesRobust, 3)))
holmForExportAdj = list(c("Test" = "Holm (p-value)", round(holmPValuesRobustAdj, 3)))
FDRForExportAdj = list(c("Test" = "BH-FDR (p-value)", round(FDRPValuesRobustAdj, 3)))

#Export cost adjusted
stargazer::stargazer(modelsAdj,                     
                     dep.var.labels	= namesForStargazer, 
                     dep.var.caption= "Dependent variable: Excess return", 
                     title ="Fama-French six-factor model: All models with robust standard errors (Cost adjusted)",
                     report=("vc*t"),
                     add.lines = c(turnoverForReporting, holmForExportAdj, FDRForExportAdj),
                     digits = 2, #digits.extra =1,
                     se  = robustErrorsCA,
                     type="text", out = paste0("Results/CostAdjustedModelsWithRobustErrorsAndCorrections", exportN, ".html")
                     )



```


```{r RNN Keras}


model <- keras_model_sequential() 
model %>% 
         layer_embedding(input_dim = 500, output_dim = 32) %>%
         layer_simple_rnn(units = 32) %>%  
         layer_dense(units = 1, activation = "sigmoid")

```


```{r LSTM Keras}

model <- keras_model_sequential() 
model %>% 
         layer_embedding(input_dim = 500, output_dim = 32) %>%
         layer_lstm(units = 32) %>%  
         layer_dense(units = 1, activation = "sigmoid")

```


```{r ForecastPlot}



#dccForecast = dccforecast(triGarch, n.roll = triForecastLength)

```




```{r OutputSummary}

#For nicer output (printing)
comparisonMatrix = matrix(c(NN1MSE, NN1MAE, 
                            lassoMSE, lassoMAE), 
                          ncol=2,
                          byrow=TRUE)
comparisonMatrix = round(comparisonMatrix, 4)

colnames(comparisonMatrix) = c("MSE", "MAE")
rownames(comparisonMatrix) = c("Neural network with one hidden layer", 
                               "Linear regression with Lasso regularisation")
print(comparisonMatrix)


```




