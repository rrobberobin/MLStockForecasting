...
system.time(fread("Data/OriginalData/FundamentalsQuarterly.csv", select=c("naics")))
system.time(fread("Data/OriginalData/FundamentalsQuarterly.csv", nThread=4, select=c("naics")))

a = fread("Data/OriginalData/FundamentalsQuarterly.csv", nThread=4, select=c("naics"="character"))
...
for(lag in 1:4){
  quarterlyReturnData[, paste("quarterlyReturns", lag, sep = "-") := c(rep(NA, min(lag, length(quarterlyReturns))), 
                                                                       head(quarterlyReturns, -lag)), by = gvkey]
  
  quarterlyReturnData[, paste("quarterlyVolatility", lag, sep = "-") := c(rep(NA, min(lag, length(quarterlyVolatility))),
...
for(lag in 2:5){
  quarterlyReturnData[, paste0("past", lag, "YearReturn") 
                      := (Reduce(`*`, shift(quarterlyReturns/100 + 1, 0:(lag*4), type="lag")) - 1)*100, by = gvkey][]
...
laggedVolAndRetNames = c("quarterlyReturns-1", "quarterlyReturns-2", "quarterlyReturns-3", "quarterlyReturns-4", "quarterlyVolatility-1",  "quarterlyVolatility-2", "quarterlyVolatility-3", "quarterlyVolatility-4", "past2YearReturn", "past3YearReturn", "past4YearReturn", "past5YearReturn", "past2YearVolatility", "past3YearVolatility", "past4YearVolatility", "past5YearVolatility")
