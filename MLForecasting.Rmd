...
NNfunction = function(layers, hidUnits, actvFunc, dropout, l1, l2, epochs) {
  lassoError = c()
  ...
  modnn <- nn_module(
    initialize = function(inputSize) {
      self$hidden <- nn_linear(inputSize, hidUnits)
  ...
  for(n in 1:validationSize) {
    ...
    fitted <- modnn %>%
      fit(
        data = list(trainX, matrix(trainY, ncol = 1)),
        valid_data = list(validX, matrix(validY, ncol = 1)),
        epochs = epochs
      )
    
    
    #MSE and MAE (The same ones as above)
    pred = predict(model, newx = validX)
...
layers = c(1, 2, 3, 4)
hidUnits = c(1, 2, 3)
actvFunc = c("relu", "sigmoid") #softmax, etc. https://keras.io/api/layers/activations/
dropout = c(0.5, 0)
regGrid = c(10^seq(3,-10, length=4), 0)
epochs = c(3, 5) #Just choose best model. => Run for 5 epochs but if third epoch is better, choose it

hyperParams = expand.grid(
  "layers" = layers,
  "hidden units" = hidUnits,
  "activationFunc" = actvFunc,
  "dropout" = dropout,
  "l1" = regGrid,
  "l2" = regGrid,
  "epochs" = epochs
