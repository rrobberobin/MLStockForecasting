...
#Import price data
require(data.table)
prices <- fread("Data/CompustatPrices.csv")
#221 835 959 x 6
...
end = start + 7  #56753 with 1 #65325 with 2 #65 331 with 3 #65 333 with 4 #65 334 with 5
...
mergedData = mergedData[order(mergedData$gvkey, mergedData$iid, -mergedData$datadate, -mergedData$fdateq),]
...
returnData = cbind("returns" = returns, "MktCap" = MktCap[-1], finalPriceData[-1, ])
...
fwrite(finalData,"Data/finalData.csv", row.names = FALSE)
...
model <- keras::load_model_tf('Results/BestKerasModel3')
