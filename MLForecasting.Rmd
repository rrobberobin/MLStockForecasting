...
fs::dir_tree("Results/BestKerasModel9")

#library(tensorflow)
library(keras)


model <- keras::load_model_tf('Results/BestKerasModel10')
#model2 <- keras::load_model_tf('Results/BestKerasModel9_2') #load second best model
#model3 <- keras::load_model_tf('Results/BestKerasModel9_3') #load third best model
...
for(q in quarters){
  ...
  allPerformances = cbind(allPerformances, modelPerformance)
  colnames(allPerformances)[m] = qName
  m = m + 1
}
...
pooledForVariance = trainValidation[, c("gvkey", "fdateYQ", "returns")]
...
monthlyReturnsWithPortfolios = monthlyReturnsWithPortfolios[, ":=" ("equalWeight"= 1/length(gvkey)), by = fdateYQ]
...
realizedPortfolios = do.call(rbind, portfolioWithRealizedList)
realizedPortfolios[, "oneReturn" := weight * monthlyReturns]
realizedPortfolios[, "portfolioReturn" := sum(oneReturn), by = c("YM")]
realizedPortfolios[, "weightCheck" := sum(weight), by = c("YM")]
realizedPortfolios$`Excess Return` = realizedPortfolios$portfolioReturn
realizedPortfolios = unique(realizedPortfolios[, c("YM", "Excess Return")], by=c("YM"))

factorsWithReturns = merge(realizedPortfolios, monthlyFactors, by.x = "YM", by.y="YM", all = TRUE)
#factorsWithReturns$excessReturn = factorsWithReturns$r - factorsWithReturns$RF


#Check excess return and remove the RFs
f = `Excess Return` ~ `Mkt-RF.DM` + `SMB.DM` + `HML.DM` + `RMW.DM` + `CMA.DM`+`RF.DM` + `WML.DM` + `Mkt-RF.EM` + `SMB.EM` + `HML.EM` + `RMW.EM` + `CMA.EM` + `RF.EM` + `WML.EM`
